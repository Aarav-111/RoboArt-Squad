...
# Kid Mode and its sounds are always on — see self.kid_mode / self.kid_sounds.
...
USER_MANUAL_SECTIONS = [
    ("Getting Started", [
        "Open Settings (⚙, top-left) and choose the serial port your robot "
        "is connected to before sending anything to it — the port list has "
        "a Scan button if your robot doesn't show up right away.",
        "On launch, a Design Options popup asks how you want to start: a "
        "predesigned rangoli, Learn Mode, Picture to Rangoli, AI Generated, "
        "or drawing freehand on the canvas. The same popup can be reopened "
        "any time from the toolbar.",
        "The Features button (toolbar) is the home for everything beyond "
        "the canvas: Learn Mode, Picture to Rangoli, and the Kolam Notebook "
        "digitizer all launch from there.",
        "Your language can be changed in Settings — every label in the app "
        "re-translates immediately, and the choice is remembered next time "
        "you open the app.",
    ]),
    ("Drawing on the Canvas", [
        "Click and drag to place and edit shapes; right-click a shape for "
        "Move / Delete.",
        "Use Mirror Mode and the pen tool (in the Design Options popup) for "
        "symmetric hand-drawn strokes.",
        "Clicking the Pen button asks how: Draw with Pen starts plain "
        "freehand strokes, or Draw using Dots lays an evenly-spaced guide "
        "grid (2×2 up to 10×10) to trace around — the dots stay on the "
        "canvas as a visual aid but are never included in what gets sent "
        "to the robot or saved.",
        "Select multiple strokes with a drag-band, then move, copy, or "
        "delete them together.",
        "Clear wipes the canvas; Send to Bot streams the current design to "
        "the robot over serial.",
        "💾 Save adds the current canvas to My Designs in the gallery, "
        "under a name you choose, so it can be reloaded in a later session.",
    ]),
    ("Sending to the Robot", [
        "Send to Bot starts streaming G-code; Pause/Cancel appear while a "
        "print is running.",
        "The progress bar and Debug Log (top-left) show streaming status "
        "and any errors from the robot.",
        "If the design uses multiple colors, the app pauses and prompts for "
        "a pen/color change between passes.",
    ]),
    ("Gallery", [
        "Opens from the toolbar or the Design Options popup. Click any card "
        "to place that design on the canvas.",
        "Pre-designed rangoli sit at the top, followed by ★ My Designs "
        "(anything you saved from the canvas), and finally one 📖 Notebook "
        "section per digitized kolam book, pages in order.",
        "Every card except the pre-designed ones has a ✕ in its corner to "
        "delete it — this asks for confirmation and cannot be undone.",
    ]),
    ("Learn Mode", [
        "Guided, step-by-step lessons that teach hand-drawing rangoli. The "
        "robot draws one part of the design and you copy it by hand with "
        "your powder bottle, alternating part by part.",
        "Features > Learn Mode has two buttons: Start (begins or resumes a "
        "lesson) and 📊 Progress, which opens your sticker book — day "
        "streak, level, and which badges are earned so far.",
        "Three lesson types are offered when you pick a design: the whole "
        "design part-by-part, a mirror/symmetry challenge (you draw the "
        "reflection of the half the robot draws), or the one-continuous-"
        "line visualisation.",
        "Difficulty is adaptive: your level (1–5) moves up or down based on "
        "your last few AI-scored attempts, and is saved between sessions — "
        "reaching Level 5 means the robot lays only guide dots.",
        "The final step can install a USB camera clamped over the mat to "
        "photograph your finished rangoli and have it scored automatically.",
        "The cartoon mascot, stickers, sounds, and confetti (Kid Mode) are "
        "on by default and only ever appear inside Learn Mode — the rest of "
        "the app keeps its normal look.",
    ]),
    ("Kolam Notebook", [
        "Features > Kolam Notebook > Start Notebook digitizes the pages of "
        "a hand-drawn kolam puthagam into designs the robot can draw, using "
        "your webcam or an uploaded photo — everything runs locally with "
        "OpenCV, nothing is uploaded anywhere.",
        "If you have no notebooks yet, Start Notebook asks whose book it "
        "is (e.g. \"Paati's Kolam Puthagam\") and goes straight to the "
        "camera, starting at page 1.",
        "If you already have one or more notebooks, Start Notebook instead "
        "opens a chooser listing each book with its page count. Continue "
        "resumes that book's next page in the camera dialog; Import opens "
        "the gallery filtered to just that book's already-digitized pages "
        "so one can be placed on the canvas. + Start New Notebook begins a "
        "brand new book alongside the existing ones.",
        "Lay the page flat, fill the frame, and keep the lighting even, "
        "then Capture Page, and either Retake or Use This Photo. Each "
        "accepted page is traced for its pulli (dot) grid and lines, shown "
        "in a review popup, and saved into the gallery under that book's "
        "📖 Notebook section — or Skip Page if a capture came out badly.",
        "Finish Notebook (or closing the camera dialog) ends the sitting "
        "and reports how many pages were saved; the count is remembered so "
        "a later sitting on the same book resumes numbering correctly.",
    ]),
    ("Picture to Rangoli / AI Generated", [
        "Picture to Rangoli takes a photo with your computer's webcam and "
        "asks OpenAI to sketch a rangoli inspired by it.",
        "AI Generated Rangoli lets you type a theme, or answer a short quiz, "
        "and OpenAI draws a matching design.",
        "Both require an OpenAI API key, entered when prompted.",
    ]),
    ("Language & Translation", [
        "Settings has a language picker covering English plus nine Indian "
        "languages; picking one re-translates every visible label right "
        "away using a free, unauthenticated translation lookup.",
        "Translations are cached to disk per language the first time each "
        "string is seen, so the app works fully offline afterwards, and "
        "never re-hits the network for a string it has already translated.",
        "If the translation lookup can't be reached (no internet), the app "
        "silently falls back to English for anything not already cached — "
        "it never blocks or errors out because of a missing translation.",
        "The Debug Log panel is deliberately never translated: it's a "
        "technical log meant to stay in English regardless of UI language.",
    ]),
    ("Achievements", [
        "Badges are unlocked automatically for milestones such as your "
        "first rangoli, a high score, a 3-symmetry-challenge streak, "
        "solving the one-line trick, reaching Level 5, or a 7-day practice "
        "streak — see them any time in Learn Mode's 📊 Progress popup.",
    ]),
    ("Troubleshooting", [
        "\"No camera found\": check the USB cable, then use the Scan button "
        "on the camera picker.",
        "\"Choose a serial port first\": open Settings and select the port "
        "before pressing Send to Bot.",
        "\"API key rejected\": re-enter a valid OpenAI key when prompted by "
        "Picture to Rangoli or AI Generated.",
        "\"OpenCV not available\" in Kolam Notebook: run "
        "pip install opencv-python, then reopen the feature.",
        "\"No kolam lines found on that page\": add more light, lay the "
        "page flatter, and make sure the pulli dots are clearly visible "
...
class ShapeApp:
    def __init__(self, root):
        ...
        # A soft click cue on every left-click, anywhere in the app — bind_all
        # rather than wiring each button individually, since it's not gated by
        # Kid Mode and is meant to cover every current and future button/card.
        self._click_sound_busy = False
        self.root.bind_all("<Button-1>", lambda e: self._click_sound(), add="+")
        ...
        self._notebook_chooser_popup = None
        ...
        # ── Kid Mode ────────────────────────────────────────────────────────
        self.kid_mode      = True         # cartoon skin over Learn Mode — always on
        self.kid_sounds    = True         # beeps on part completion — always on
        ...
        self._pen_dot_grid_n = None   # last chosen guide-dot grid, for resize
    ...
    def _apply_canvas_resize(self):
        ...
        self.draw_grid()
        self.redraw()
        if getattr(self, "_pen_dot_grid_n", None):
            self._draw_pen_dot_grid(self._pen_dot_grid_n)
    ...
    def _build_banner(self, parent):
        ...
        self.settings_btn.pack(side="left", padx=(S(8), S(0)))

        self.help_btn = self._color_button(
            controls, "❓", self._open_help_popup, "#1e293b",
            width=S(40), height=S(36), font_size=FS(15), corner_radius=S(18))
        self.help_btn.pack(side="left", padx=(S(8), S(0)))
    ...
    def _open_design_options_popup(self):
        ...
        self.pen_btn = self._small_btn(
            pen_row, self._pen_btn_label(), _pick(self._pen_btn_click),
    ...
    def _open_help_popup(self):
        ...
        self.root.update_idletasks()
        W = min(S(560), self.root.winfo_screenwidth() - S(40))
        ...
        self._draw_rounded_rect(glass, 4, 4, W-4, H-4, radius=S(20),
                                fill=BG_CARD, outline=ACCENT_PURP, width=2)
        glass.create_text(24, 26, text=self.tr("User Manual"), anchor="w",
                          fill=TEXT_PRIMARY, font=("Segoe UI", FS(14), "bold"))

        close_lbl = tk.Label(popup, text="✕", bg=BG_CARD, fg=TEXT_DIM,
                             font=("Segoe UI", FS(13), "bold"), cursor="hand2")
        close_lbl.place(x=W-38, y=14)
        close_lbl.bind("<Button-1>", lambda e: self._close_help_popup())

        body = tk.Frame(popup, bg=BG_CARD)
        body.place(x=20, y=54, width=W-40, height=H-74)

        text_scroll = tk.Scrollbar(body)
        text_scroll.pack(side="right", fill="y")
        text_box = tk.Text(
            body, bg=BG_INPUT, fg=TEXT_PRIMARY, wrap="word",
            font=("Segoe UI", FS(10)), relief="flat", padx=S(10), pady=S(10),
            yscrollcommand=text_scroll.set, highlightthickness=0, bd=0)
        text_box.pack(side="left", fill="both", expand=True)
        text_scroll.config(command=text_box.yview)

        text_box.tag_configure(
            "h1", font=("Segoe UI", FS(12), "bold"), foreground=ACCENT_AMBER,
            spacing1=S(10), spacing3=S(4))
        text_box.tag_configure(
            "body", font=("Segoe UI", FS(10)), spacing3=S(6))

        for heading, lines in USER_MANUAL_SECTIONS:
            text_box.insert("end", self.tr(heading) + "\n", "h1")
            for line in lines:
                text_box.insert("end", "• " + self.tr(line) + "\n", "body")
    ...
    def _open_features_popup(self):
        ...
        # Learn Mode — guided, step-by-step rangoli lessons. Given its own
        # sub-frame (rather than living loose in `body` like the other
        # rows) so _kid_skin_tree below can reskin just this section in the
        # playful Kid Mode palette/font without touching Notebook / Picture
        # to Rangoli / Freehand Pen beneath it.
        learn_section = tk.Frame(body, bg=BG_CARD)
        learn_section.pack(fill="x", pady=(S(0), S(14)))
        row = tk.Frame(learn_section, bg=BG_CARD)
        row.pack(fill="x", pady=(S(0), S(6)))
        tk.Label(row, text="🧚 " + self.tr("Learn Mode"), bg=BG_CARD,
                 fg=TEXT_PRIMARY,
                 font=("Segoe UI", FS(13), "bold")).pack(side="left")
        btn_box = tk.Frame(row, bg=BG_CARD)
        btn_box.pack(side="right")
        # _small_btn always packs itself side="right" within its parent, so
        # the button packed *last* ends up visually leftmost — pack Progress
        # first to land [Start] [Progress] left-to-right as asked for.
        self._small_btn(
            btn_box, "📊 " + self.tr("Progress"), self._open_kid_stickers,
            ACCENT_PINK, "#f472b6")
        on = self.learn_mode_var.get()
        self.learn_btn = self._small_btn(
            btn_box, ("🛑 " if on else "▶ ") +
            (self.tr("Stop") if on else self.tr("Start")), self._toggle_learn_mode,
            ACCENT_PINK if on else ACCENT_GREEN, "#f472b6" if on else "#4ade80")
        tk.Label(learn_section,
        ...
        if self._kid_earned_badges() or self._kid_streak():
            tk.Label(learn_section, text=self._kid_progress_line(),
                     bg=BG_CARD, fg=ACCENT_AMBER,
                     font=("Segoe UI", FS(9), "bold")).pack(anchor="w",
                                                            pady=(S(4), 0))
        self.root.after(0, lambda: self._kid_skin_tree(learn_section))
        ...
        tk.Label(nb_row, text=self.tr("Kolam Notebook"), bg=BG_CARD, fg=TEXT_PRIMARY,
                 font=("Segoe UI", FS(12), "bold")).pack(side="left")
        self._small_btn(
            nb_row, self.tr("Start Notebook"), self._launch_notebook_digitizer,
    ...
    def _launch_notebook_digitizer(self):
        """Features popup > Kolam Notebook > Start Notebook. Close the popup
        first so the chooser/capture dialog's grab isn't fighting an
        already-open Toplevel."""
        self._close_features_popup()
        self.root.after(10, self._open_notebook_chooser)

    def _notebook_books(self):
        """Existing notebook names -> page count, newest additions first
        within each book, sorted by name for a stable chooser order."""
        books = {}
        for _name, _full, data in self._load_saved_designs():
            nb = data.get('notebook')
            if isinstance(nb, dict) and nb.get('book'):
                books[nb['book']] = books.get(nb['book'], 0) + 1
        return sorted(books.items())

    def _open_notebook_chooser(self):
        """Start Notebook's landing point: skip straight to naming a first
        notebook when none exist yet; otherwise offer to import from or
        continue an old one, or start a brand new one alongside them."""
        books = self._notebook_books()
        if not books:
            self._notebook_start_new()
            return

        self._close_notebook_chooser_popup()
        self.root.update_idletasks()
        W = S(440)
        H = S(130 + 56 * len(books) + 74)
        sx = self.root.winfo_screenwidth()  // 2 - W // 2
        sy = self.root.winfo_screenheight() // 2 - H // 2
        ...
        popup.configure(bg=BG_DARK)
        popup.transient(self.root)
        self._notebook_chooser_popup = popup
        ...
        glass.create_text(24, 50,
                          text=self.tr("Import an old notebook's pages, continue "
                               "one, or start a new sitting."),
                          anchor="w", fill=TEXT_DIM, font=("Segoe UI", FS(9)))
        glass.bind("<Button-1>", lambda e: "break")
        ...
        close_lbl = tk.Label(popup, text="✕", bg=BG_CARD, fg=TEXT_DIM,
                             font=("Segoe UI", FS(13), "bold"), cursor="hand2")
        close_lbl.place(x=W-38, y=14)
        close_lbl.bind("<Button-1>", lambda e: self._close_notebook_chooser_popup())
        ...
        for book, count in books:
            ...
            info.pack(side="left", fill="y", padx=(S(10), S(4)))
            tk.Label(info, text=book, bg=BG_INPUT, fg=TEXT_PRIMARY,
                     font=("Segoe UI", FS(11), "bold")).pack(anchor="w",
                                                             pady=(S(8), S(0)))
            tk.Label(info,
                     text=f"{count} " + self.tr("page" if count == 1 else "pages"),
                     bg=BG_INPUT, fg=TEXT_DIM, font=("Segoe UI", FS(9))
                     ).pack(anchor="w", pady=(S(0), S(8)))
            self._small_btn(
                row, self.tr("Continue"), lambda b=book: self._notebook_continue(b),
                ACCENT_GREEN, self._lighten(ACCENT_GREEN, -30))
            self._small_btn(
                row, self.tr("Import"), lambda b=book: self._notebook_import(b),
                ACCENT_PURP, self._lighten(ACCENT_PURP, -30))

        self._color_button(
            body, "+ " + self.tr("Start New Notebook"), self._notebook_start_new,
            ACCENT_AMBER, width=W-60, height=S(40), font_size=FS(11)
        ).pack(side="bottom", pady=(S(10), 0))

        popup.lift()
        popup.focus_force()
        popup.grab_set()

    def _close_notebook_chooser_popup(self):
        popup = self._notebook_chooser_popup
        self._notebook_chooser_popup = None
        if popup is None:
            return
        try: popup.grab_release()
    ...
    def _notebook_import(self, book):
        """Chooser > Import: open the gallery filtered to just this book's
        digitized pages, so one can be placed on the canvas or sent to the
        robot like any other saved design."""
        self._close_notebook_chooser_popup()
        self.show_gallery_popup(notebook_filter=book)

    def _notebook_continue(self, book):
        """Chooser > Continue: resume this book's sitting without asking for
        its name again."""
        self._close_notebook_chooser_popup()
    ...
    def _notebook_start_new(self):
        """Chooser > + Start New Notebook (also the direct path when no
        notebook exists yet): ask whose book this is, then start page 1."""
        self._close_notebook_chooser_popup()
        dlg = ctk.CTkInputDialog(
            text="Whose notebook is this?\n(e.g. Paati's Kolam Puthagam)",
            title="Kolam Notebook — New Sitting")
        # Launched via root.after() right after the Features popup closes, so
        # the window never became the OS-level key window — CTk's own
        # entry.focus() (at +150ms) only sets Tk focus, not that. Force it
        # after CTk's delay so keystrokes actually land in the entry.
    ...
    def _open_notebook_capture_dialog(self):
        ...
        def do_use():
            ...
            if frame is None:
                return
            on_close()
            self._digitize_notebook_frame(frame)
    ...
    def _nb_process_page(self, frame, snap=True, remove_rules=True):
        ...
        h0, w0 = gray.shape[:2]
        sc = NB_MAX_DIM / float(max(h0, w0))
        if sc < 1.0:
            gray = cv2.resize(gray, (max(1, int(w0 * sc)), max(1, int(h0 * sc))),
        ...
        if remove_rules:
            ...
            rules = cv2.bitwise_or(
                self._nb_rule_mask(ink, horizontal=True),
                self._nb_rule_mask(ink, horizontal=False))
            if cv2.countNonZero(rules):
                rules_removed = True
                ink = cv2.subtract(ink, rules)

        # Dots come off the page before any repair work: a pulli sits within a
        # few pixels of the line it belongs to, and a closing wide enough to
        # mend a stroke is wide enough to weld the dot onto it.
        dots, dot_mask = self._nb_detect_dots(ink, gray)
        lines = cv2.subtract(ink, dot_mask)
        if rules_removed:
            # Every rule crossed the kolam dozens of times, so taking the rules
            # out has left the design in arcs. Closing *across* each band —
            # vertically for horizontal rules and the other way round for
            # vertical ones — rejoins a stroke that continued on both sides,
            # without smearing anything along the band itself.
        ...
        pitch, angle, grid_ok = None, 0.0, False
        fit = self._nb_fit_grid(dots)
        if fit is not None:
            snapped, src, pitch, angle, keep = fit
            # Only the dots that sit on the lattice are pulli; the rest were
            # page grain and go no further.
            dots = [d for d, ok in zip(dots, keep) if ok]
    ...
    @staticmethod
    def _nb_rule_mask(ink, horizontal=True, iters=48):
        ...
        # Long, and with a tight gap tolerance: loosen either and the near-
        # vertical tangent that every ring of a kolam offers up gets threaded
        # into a "line" running the height of the drawing.
        min_len = max(60, span // 4)
        mask = np.zeros_like(ink)
        lines = cv2.HoughLinesP(ink, 1, np.pi / 360.0,
                                threshold=max(40, min_len // 3),
                                minLineLength=min_len,
                                maxLineGap=max(8, span // 40))
        if lines is None:
            return mask
        ...
        # behind — a tidy row of specks that looks just like pulli.
        reach = cv2.morphologyEx(ink, cv2.MORPH_CLOSE, join)
        ...
        for _ in range(iters):
            ...
            if cv2.countNonZero(cv2.absdiff(grown, seed)) == 0:
                break
            seed = grown
        return cv2.bitwise_and(seed, ink)

    def _nb_detect_dots(self, ink, gray):
        """Pulli dots as their own layer, separate from the lines that join them.

        Connected components first — a pencil pulli is a small solid blob, and
        that is far more reliable on paper than gradient circle-finding.
        HoughCircles backs it up when the dots are faint or nearly all of them
        are touched by a line (a touched dot merges into its stroke's component
        and is invisible to the blob pass).

        Returns (dots, dot_mask) where dots is [(x, y, radius), ...]."""
        import cv2
        import numpy as np
        ...
        cand = []
        for i in range(1, n):
            x, y, bw, bh, area = (int(stats[i][0]), int(stats[i][1]),
        ...
        dots = list(cand)
        if len(cand) >= 4:
            r_med = float(np.median([c[2] for c in cand]))
            # Pulli on one page are all the same size; anything far off that
            # is a smudge or a full stop, not a dot.
            dots = [c for c in cand if 0.45 * r_med <= c[2] <= 2.2 * r_med]

        dot_mask = np.zeros_like(ink)
        if dots:
            keep = np.zeros(n, dtype=bool)
            for _x, _y, _r, lab in dots:
                keep[lab] = True
            dot_mask[keep[labels]] = 255

        found = [(x, y, r) for x, y, r, _lab in dots]
        if len(found) >= 4:
            return found, dot_mask
        ...
        for c in circles[0]:
            ...
            if any((cx - fx) ** 2 + (cy - fy) ** 2 < min_dist ** 2
                   for fx, fy, _fr in found):
                continue
            found.append((cx, cy, r))
            cv2.circle(dot_mask, (int(round(cx)), int(round(cy))),
                       int(round(r)) + 1, 255, -1)
        return found, dot_mask

    def _nb_trace_strokes(self, lines_mask):
        """Outline every connecting line left after the dots are taken out.

        Contours trace around a stroke rather than down its middle, so a
        pencil line comes back as a long thin loop — which draws identically
        in powder and needs no thinning library to be installed."""
        import cv2

        h, w = lines_mask.shape[:2]
        contours, _ = cv2.findContours(lines_mask, cv2.RETR_LIST,
        ...
        for cnt in contours:
            ...
            if per < min_len:
                continue
            approx = cv2.approxPolyDP(cnt, max(1.0, 0.0015 * per), True)
            if len(approx) < 3:
                continue
            pts = [(float(p[0][0]), float(p[0][1])) for p in approx]
            pts.append(pts[0])
            scored.append((per, pts))
        scored.sort(key=lambda t: t[0], reverse=True)
        return [pts for _per, pts in scored[:NB_MAX_STROKES]]
    ...
    def _nb_lattice(pts, pitch):
        ...
        return out, fitted_pitch, angle, int(inliers.sum()), inliers
    ...
    def _open_notebook_review_popup(self, frame, result, opts):
        ...
        status_lbl = tk.Label(popup, text="", bg=BG_CARD, fg=TEXT_DIM,
                              font=("Segoe UI", FS(9), "bold"))
        status_lbl.place(x=S(28), y=prev_y + CW + S(8))
        ...
        # ── toggles ─────────────────────────────────────────────────────────
        tog_y = prev_y + CW + S(30)
        toggles = tk.Frame(popup, bg=BG_CARD)
        toggles.place(x=S(28), y=tog_y)

        def _toggle_btn(text, cmd, on):
            b = ctk.CTkButton(
                toggles, text=text, command=cmd, width=S(150), height=S(30),
        ...
        def toggle_edit():
            ...
            redraw_preview()
        edit_btn.configure(command=toggle_edit)
        edit_btn.place(x=S(28), y=H - S(56))

        def _foot_btn(text, cmd, colour, x):
            ctk.CTkButton(
                popup, text=text, width=S(128), height=S(32),
                fg_color="transparent", hover_color=BG_INPUT, border_width=1,
                border_color=colour, text_color=colour,
                font=("Segoe UI", FS(10), "bold"), command=cmd
                ).place(x=x, y=H - S(56))

        _foot_btn("Skip Page", self._notebook_skip_page, TEXT_DIM, W - S(420))
        _foot_btn("Place on Canvas",
                  lambda: self._notebook_place_page(state), ACCENT_PURP,
                  W - S(284))
        _foot_btn("Save & Next Page →",
    ...
    def _get_openai_api_key(self):
        """Key lives in openai_key.txt next to the script — never in source,
        so it can't end up committed to git or shared with the file. Read it
        if present; otherwise ask once and save it for next time."""
        try:
        ...
        dlg = ctk.CTkInputDialog(
            text="Enter your OpenAI API key\n"
                 "(saved locally in openai_key.txt, never uploaded anywhere):",
            title="OpenAI API Key")
        # See Kolam Notebook's naming dialog for why this delayed
        # lift()/focus_force() is needed — CTk's own focus() at +150ms only
        # grabs Tk focus, not the OS-level key window.
        dlg.after(200, lambda: (dlg.lift(), dlg.focus_force()))
    ...
    def show_gallery_popup(self, notebook_filter=None):
        """``notebook_filter``: when set (from Kolam Notebook's chooser >
        Import), show only that book's digitized pages instead of the whole
        gallery."""
        self._close_gallery_popup()
        self.root.update_idletasks()
        ...
        if notebook_filter:
            glass.create_text(28, 30, text=f"\U0001f4d6 Notebook — {notebook_filter}",
                              anchor="w", fill=TEXT_PRIMARY,
                              font=("Segoe UI", FS(16), "bold"))
            glass.create_text(28, 54, text="Click a page to place it on the "
                              "canvas.", anchor="w", fill=TEXT_DIM,
                              font=("Segoe UI", FS(9)))
        else:
            ...
            glass.create_text(28, 54, text="Click a design to place it. "
                              ...
                              "\U0001f4be Save button; \U0001f4d6 Notebook holds "
                              "digitized kolam book pages.",
                              anchor="w", fill=TEXT_DIM, font=("Segoe UI", FS(9)))
        ...
        if notebook_filter:
            library = {}
            saved = []
            notebook = [t for t in notebook
                        if t[2]['notebook'].get('book') == notebook_filter]
        else:
        ...
        def _saved_cards(r, entries, label_of):
        ...
        if not library and not saved and not notebook:
            ...
            if notebook:
                ...
                for book in sorted(books):
                    ...
                    row = _section_header(
                        row, f"\U0001f4d6 Notebook — {book}", ACCENT_AMBER)
                    row = _saved_cards(
                        row, pages,
    ...
    # ── Sound: cross-platform tone playback ──────────────────────────────────
    # winsound (Beep) only exists on Windows, so a beep synthesized there is
    # silent everywhere else. Every platform instead gets a tiny sine-wave WAV
    # rendered on the fly and handed to the OS' own player — no extra audio
    # library needed just for a beep.
    def _play_tone_sequence(self, notes):
        """``notes``: [(freq_hz, ms), …], played back-to-back as one sound."""
        def _play():
            try:
                if sys.platform == "win32":
                    ...
                    return
                import numpy as np, wave, tempfile, subprocess
                rate = 44100
                chunks = []
                for freq, ms in notes:
                    ...
                    # short fade in/out so notes don't click at the seams
                    fade = min(n // 8, 200)
                    if fade:
                        env = np.ones(n)
                        ...
                        tone *= env
                    chunks.append((tone * 0.25 * 32767).astype(np.int16))
                ...
                player = (["afplay", path] if sys.platform == "darwin"
                          else ["aplay", "-q", path])
                ...
                pass  # a missing sound device must never interrupt anything
        threading.Thread(target=_play, daemon=True).start()

    def _click_sound(self):
        """Soft click cue for every button/card press across the whole app,
        independent of Kid Mode — throttled so a rapid double-click or a
        drag-band selection can't pile up overlapping players."""
        if getattr(self, "_click_sound_busy", False):
            return
        self._click_sound_busy = True
        def _clear():
            self._click_sound_busy = False
        self._play_tone_sequence([(1500, 18)])
        self.root.after(60, _clear)

    # ── Kid Mode: sounds (muted unless switched on) ───────────────────────────
    def _kid_sound(self, kind):
        """Small musical cue for a Learn Mode moment. No-op unless Kid Mode
        AND sounds are both on (both default True, see __init__)."""
        ...
        if not notes:
            return
        self._play_tone_sequence(notes)
    ...
    def _pen_btn_click(self):
        """Design Options > Freehand Pen button. Turning pen mode off is
        immediate (same as before); turning it on now asks how — plain
        strokes, or with a dot grid to trace around."""
        if self.pen_mode_var.get():
            self.toggle_pen_mode()
            return
        self._open_pen_choice_popup()

    def _guarded(self, flag_attr, fn):
        """Wrap a just-opened popup's button command so a click can't fire
        until `flag_attr` is armed a beat later.

        Two topmost overrideredirect popups opening in quick succession at
        nearly the same screen position (the Pen choice popup, then the grid
        popup right after it) let the *same* physical click land on the new
        window before the user has actually seen it — button 10x10, being
        last in that grid, was on the receiving end of exactly that, and the
        first popup being caught by it too is why it never appeared to open
        at all."""
        def wrapped(*a, **k):
    ...
    def _open_pen_choice_popup(self):
        self._close_pen_choice_popup()
        self.root.update_idletasks()
        W, H = S(360), S(230)
        sx = self.root.winfo_screenwidth()  // 2 - W // 2
        sy = self.root.winfo_screenheight() // 2 - H // 2
        ...
        popup.configure(bg=BG_DARK)
        popup.transient(self.root)
        self._pen_choice_popup = popup
        self._pen_choice_armed = False
        ...
        glass.create_text(24, 26, text=self.tr("Freehand Pen"), anchor="w",
                          fill=TEXT_PRIMARY, font=("Segoe UI", FS(14), "bold"))
        glass.create_text(24, 50, text=self.tr("How would you like to draw?"),
                          anchor="w", fill=TEXT_DIM, font=("Segoe UI", FS(9)))
        glass.bind("<Button-1>", lambda e: "break")
        ...
        close_lbl = tk.Label(popup, text="✕", bg=BG_CARD, fg=TEXT_DIM,
                             font=("Segoe UI", FS(13), "bold"), cursor="hand2")
        close_lbl.place(x=W-38, y=14)
        close_lbl.bind("<Button-1>", lambda e: self._close_pen_choice_popup())
        ...
        pen_choice_buttons = [
            self._color_button(
                body, "✏ " + self.tr("Draw with Pen"),
                self._guarded("_pen_choice_armed", self._choose_pen_plain),
                ACCENT_CYAN, width=W-60, height=S(48), font_size=FS(12)),
            self._color_button(
                body, "⚄ " + self.tr("Draw using Dots"),
                self._guarded("_pen_choice_armed", self._choose_pen_dots),
                ACCENT_AMBER, width=W-60, height=S(48), font_size=FS(12)),
        ]
        pen_choice_buttons[0].pack(pady=(S(0), S(12)))
        pen_choice_buttons[1].pack()

        # See _open_pen_grid_popup for why this starts disabled rather than
        # silently swallowing an early click: a 300ms silent guard ate a fast
        # click on "Draw with Pen" (the first, closest button) with no
        # visible feedback at all.
        for b in pen_choice_buttons:
            try: b.configure(state="disabled")
            except Exception: pass

        def _arm():
            self._pen_choice_armed = True
            for b in pen_choice_buttons:
                try: b.configure(state="normal")
                except Exception: pass
        popup.after(150, _arm)

        popup.lift()
        popup.focus_force()
        popup.grab_set()

    def _close_pen_choice_popup(self):
        popup = getattr(self, "_pen_choice_popup", None)
        self._pen_choice_popup = None
        if popup is None:
            return
        try: popup.grab_release()
        except Exception: pass
    ...
    def _choose_pen_plain(self):
        self._close_pen_choice_popup()
        self.toggle_pen_mode()

    def _choose_pen_dots(self):
        self._close_pen_choice_popup()
        self.root.after(300, self._open_pen_grid_popup)

    def _open_pen_grid_popup(self):
        """Draw using Dots > which grid. Any size can be added here — the
        dots are placed on an evenly-spaced n×n grid over the drawing area."""
        self._close_pen_grid_popup()
        self.root.update_idletasks()
        sizes = [2, 4, 6, 8, 10]
        cols = 3
        ...
        popup.configure(bg=BG_DARK)
        popup.transient(self.root)
        self._pen_grid_popup = popup
        self._pen_grid_armed = False
        pen_grid_buttons = []
        ...
        self._draw_rounded_rect(glass, 4, 4, W-4, H-4, radius=S(20),
                                fill=BG_CARD, outline=ACCENT_AMBER, width=2)
        glass.create_text(24, 26, text=self.tr("Dot Grid"), anchor="w",
                          fill=TEXT_PRIMARY, font=("Segoe UI", FS(14), "bold"))
        glass.create_text(24, 50, text=self.tr("Pick a grid to trace around — "
                          "these guide dots are never sent to the robot."),
                          anchor="w", fill=TEXT_DIM, font=("Segoe UI", FS(9)),
                          width=W-48)
        glass.bind("<Button-1>", lambda e: "break")
        ...
        close_lbl = tk.Label(popup, text="✕", bg=BG_CARD, fg=TEXT_DIM,
                             font=("Segoe UI", FS(13), "bold"), cursor="hand2")
        close_lbl.place(x=W-38, y=14)
        close_lbl.bind("<Button-1>", lambda e: self._close_pen_grid_popup())
        ...
        for idx, n in enumerate(sizes):
            ...
            btn = self._color_button(
                body, f"{n}×{n}",
                self._guarded("_pen_grid_armed",
                              lambda n=n: self._apply_pen_dot_grid(n)),
                ACCENT_AMBER, width=(W-40-2*S(8))//cols - S(4), height=S(46),
                font_size=FS(12))
            btn.grid(row=r, column=c, padx=S(4), pady=S(4))
            pen_grid_buttons.append(btn)

        # Buttons start visibly disabled and flip to normal a beat later —
        # the 300ms version of this guard silently ate a genuine fast click
        # on 2×2 (the first, closest button people reach for) with zero
        # feedback, which read as "2x2 does nothing". Greyed-out-then-enabled
        # still blocks a stray click carried over from the previous popup,
        # but makes the short wait visible instead of a silent no-op.
        for b in pen_grid_buttons:
            try: b.configure(state="disabled")
            except Exception: pass

        def _arm():
            self._pen_grid_armed = True
            for b in pen_grid_buttons:
                try: b.configure(state="normal")
                except Exception: pass
        popup.after(150, _arm)
    ...
    def _close_pen_grid_popup(self):
        popup = getattr(self, "_pen_grid_popup", None)
        self._pen_grid_popup = None
        if popup is None:
            return
        try: popup.grab_release()
        except Exception: pass
    ...
    def _draw_pen_dot_grid(self, n):
        """Just the drawing part of the guide grid — no popup/mode handling,
        so it can also be called after a window resize (which wipes the whole
        canvas with delete("all")) to put a previously-chosen grid back."""
        self.canvas.delete("pen_guide_dot")
        r = S(3.5)
        # MARGIN_T and MARGIN_R are only ~4px (just room for the axis line),
        # so insetting by the dot's own radius still left the top/right edge
        # dots rendered right under the floating toolbar row and hint banner
        # that overlay the canvas — inside the 0..4 bed, but visually well
        # outside where the drawing area reads as being. A flat safety
        # padding, independent of those thin axis margins, keeps every dot
        # comfortably inside the visible white square on all four sides.
        pad = S(24) + r + 1
        x0, x1 = MARGIN_L + pad, CANVAS_W - MARGIN_R - pad
        y0, y1 = MARGIN_T + pad, CANVAS_H - MARGIN_B - pad
        for ix in range(n):
            for iy in range(n):
                px = x0 + (x1 - x0) * (ix / (n - 1) if n > 1 else 0.5)
                py = y0 + (y1 - y0) * (iy / (n - 1) if n > 1 else 0.5)
                self.canvas.create_oval(
                    px - r, py - r, px + r, py + r,
                    fill=ACCENT_AMBER, outline="#c2410c", width=1,
                    tags="pen_guide_dot")
        self.canvas.tag_raise("pen_guide_dot")

    def _apply_pen_dot_grid(self, n):
        """Draw using Dots > pick a size: lay an n×n guide grid over the
        drawing area and turn pen mode on. Guide dots use their own canvas
        tag, are never added to self.shapes, and are skipped by
        redraw()/clear_canvas()'s deletes — so they sit on the canvas purely
        as a visual aid and never reach the G-code sent to the robot or a
        saved design."""
        self._close_pen_grid_popup()
        self._pen_dot_grid_n = n
        self._draw_pen_dot_grid(n)
        self.log_to_console(
            f"Pen: {n}x{n} guide dots added — draw around them; they stay on "
            f"the canvas but are never sent to the robot.", "info")
