...
#
# The canvas->mm mapping is exactly isotropic (GRAPH_W == GRAPH_H and
# MAX_X == MAX_Y), so a commanded circle *is* a circle in machine units. If the
# robot still lays down an oval, its two axes travel different distances for the
# same commanded millimetre — i.e. GRBL's $100 (X steps/mm) and $101 (Y steps/mm)
# disagree with each other, or a belt/wheel diameter differs between axes.
#
# Fixing $100/$101 in firmware is the proper repair. These factors are the
# software workaround when that isn't practical.
#
# Values below were dialled in against a measured test circle on the real
# robot: Y is scaled down so the taller axis matches the shorter one. Scaling
# DOWN (never up) keeps the design inside the mat. 1.0 = no correction.
#
# To re-calibrate after a belt/wheel change: draw a Circle, measure the powder
# ring, and scale down whichever axis came out too long — e.g. a ring measuring
# 24cm wide x 20cm tall means CAL_X = 20/24 = 0.83, CAL_Y = 1.0.
CAL_X    = 1.0
CAL_Y    = 0.84
...
# Automatic multi-colour changing. The refill dock sits just outside the
# print bed (MAX_X is 4.7), so the robot can park under the dispenser without
# ever crossing the mat. After the operator empties the old colour at the
# origin, the head drives here on its own, holds still for REFILL_DWELL_S
# while the dispenser fills the nozzle, then returns — no manual loading.
REFILL_X       = 5.20
REFILL_Y       = 0.00
REFILL_DWELL_S = 2
...
REF_MARGIN_B = 32
REF_MARGIN_T = 4
# The right gutter is wide enough to show the automatic colour changer, which
# sits at REFILL_X — past the right edge of the bed and so off the plot area
# proper. Without the reserve the dock block would be drawn off-canvas.
REF_MARGIN_R = 64
...
# The machine carries exactly two powders: the one loaded in the nozzle and
# the one at the refill station. Offering more colours in the UI than the
# hardware can actually dispense only produces designs it cannot draw.
COLOUR_PALETTE = {
    "Red":  "#ef4444",
    "Blue": "#3b82f6",
}

# What the nozzle is loaded with at the start of every job. Anything drawn in
# this colour needs no swap; the other colour costs one trip to the refill
# station, so LOADED_COLOUR is always drawn first and the swap happens once.
LOADED_COLOUR = "Blue"
...
MAT_SPAN_MM = 240.0                 # real width of the mat the bed maps onto
...
# Only the two powders the machine actually carries, so a lesson never asks
# the student to load a colour that isn't there. Parts alternate red/blue.
LEARN_PART_COLORS = [
...
class ShapeApp:
    def __init__(self, root):
        ...
        self._thumbs_test_dlg  = None   # TEMPORARY: ⚙ Settings → Thumbs-up test
    ...
    def _open_settings_popup(self):
        ...
        # small display.
        W = S(400)
        H = min(S(790), self.root.winfo_screenheight() - S(40))
        sx = self.root.winfo_screenwidth()  // 2 - W // 2
        ...
        # 2b) Automatic colour changing — lives here rather than on the canvas
        # strip, which has no room left for it beside the colour picker.
        slot = _row("Auto colour change",
                    f"Refill at the X{REFILL_X:g} station instead of by hand.")
        self.auto_colour_switch = ctk.CTkSwitch(
            slot, text="", variable=self.auto_colour_var,
            fg_color=GLASS_EDGE, progress_color=ACCENT_AMBER,
            bg_color=BG_CARD, width=S(48), height=S(22))
        self.auto_colour_switch.pack(side="right")
        ...
        # 4) TEMPORARY — bench the Learn-Mode thumbs-up without playing a whole
        # lesson: live skeleton, every measurement, and the hold counter. Pull
        # this row (and _open_thumbs_test_dialog) once the gesture is trusted.
        slot = _row("Thumbs-up test",
                    "Temporary: check the Learn Mode thumbs-up against the "
                    "camera — hand skeleton, every measurement, live.")
        self._color_button(slot, "\U0001f44d Open tester",
                           self._open_thumbs_test_dialog, ACCENT_GREEN,
    ...
    def _draw_refill_dock(self, c, x0, x1, y1):
        """Mark the automatic colour changer in the gutter right of the bed.

        REFILL_X is deliberately outside the printable area, so the station
        has no place on the plot itself — it gets a block in the right margin
        at its true scaled X, with a dashed lead-in showing the trip the head
        makes to reach it during a colour change.
        """
        px = x0 + (REFILL_X / (MAX_X * CAL_X)) * GRAPH_W
        py = y1 - (REFILL_Y / (MAX_Y * CAL_Y)) * GRAPH_H
        half_w, half_h = S(11), S(16)
        # Never let the block run off the canvas if the gutter comes out
        # narrower than the dock overshoot on an unusual window size.
        px = min(px, CANVAS_W - half_w - S(2))

        c.create_line(x1, py, px - half_w, py, fill="#f59e0b",
                      width=S(1), dash=(3, 3), tags="grid")
        c.create_rectangle(px - half_w, py - half_h, px + half_w, py + half_h,
                           fill="#f97316", outline="#facc15", width=S(2),
                           tags="grid")
        c.create_text(px, py, text="⟳", fill="#ffffff",
                      font=("Segoe UI", FS(12), "bold"), tags="grid")
        c.create_text(px, py - half_h - S(8), text="REFILL",
                      fill="#b45309", font=("Segoe UI", FS(6), "bold"),
                      tags="grid")
        c.create_text(px, py + half_h + S(8), text=f"X{REFILL_X:g}",
                      fill="#b45309", font=("Consolas", FS(6)), tags="grid")
    ...
    def _show_colour_switch_popup(self, colour):
        ...
        ctk.CTkButton(
            body,
            text=("Emptied — refill automatically" if auto
    ...
    def show_gallery_popup(self):
        ...
        def _on_state_change(*_a):
            self._gallery_state_filter = state_var.get()
            self.show_gallery_popup()

        state_menu = tk.OptionMenu(filter_bar, state_var, *state_names,
                                    command=lambda _v: _on_state_change())
        state_menu.configure(bg=BG_INPUT, fg=TEXT_PRIMARY, activebackground=BG_INPUT,
                              highlightthickness=0, font=("Segoe UI", FS(9)),
                              relief="flat", cursor="hand2")
        state_menu["menu"].configure(bg=BG_INPUT, fg=TEXT_PRIMARY)
    ...
    # ── TEMPORARY: thumbs-up tester (⚙ Settings → Thumbs-up test) ──────────
    # A bench window for the gesture: the camera picture with the hand skeleton
    # drawn on it, every measurement the decision is made from, and the same
    # hold counter the Learn popup uses. Nothing else in the app calls it —
    # delete this method and its Settings row when the gesture is trusted.

    def _open_thumbs_test_dialog(self):
        ok, reason = self._gesture_ready()
        if not ok:
            self.log_to_console(f"Thumbs-up test: {reason}", "err")
            self.show_hint_popup(reason)
            return
        existing = getattr(self, "_thumbs_test_dlg", None)
        if existing is not None:
            try:
        ...
        dlg = tk.Toplevel(self.root)
        dlg.title("Thumbs-up test (temporary)")
        dlg.configure(bg=BG_CARD)
        dlg.resizable(False, False)
        dlg.transient(self.root)
        self._thumbs_test_dlg = dlg
        state = {"stop": threading.Event(), "frame": None, "report": None,
                 "run": 0, "err": None, "after": None, "img": None}

        pad = tk.Frame(dlg, bg=BG_CARD)
        pad.pack(fill="both", expand=True, padx=S(16), pady=S(14))
        tk.Label(pad, text="\U0001f44d Thumbs-up test", bg=BG_CARD,
                 fg=TEXT_PRIMARY, font=("Segoe UI", FS(14), "bold")).pack(anchor="w")
        tk.Label(pad, text="Every test below is measured on the hand's own "
                           "joints, so turn your hand any way you like — "
                           "sideways, tilted, upside down, either hand. Green "
                           "means that test passes right now.",
                 bg=BG_CARD, fg=TEXT_DIM, font=("Segoe UI", FS(9)),
        ...
        verdict = tk.Label(side, text="no hand", bg=BG_CARD, fg=TEXT_DIM,
                           font=("Segoe UI", FS(15), "bold"), anchor="w")
        verdict.pack(fill="x")
        hold = tk.Label(side, text="", bg=BG_CARD, fg=TEXT_DIM,
                        font=("Segoe UI", FS(9)), anchor="w")
        hold.pack(fill="x", pady=(0, S(8)))
                ...
                while not state["stop"].is_set():
                    ...
                    try:
                        res = detector.detect(mp.Image(
                            image_format=mp.ImageFormat.SRGB,
                            data=np.ascontiguousarray(rgb)))
                        # The verdict is read off the world landmarks; the
                        # image ones are only good for drawing.
                        worlds = res.hand_world_landmarks or []
                        shown  = res.hand_landmarks or []
                        if worlds:
                            report = self._thumbs_up_report(worlds[0])
                        if shown:
                            good = bool(report and report["ok"])
                            ...
                            for i, pt in enumerate(pts):
                                cv2.circle(view_bgr, pt, 5 if i in (4, 0) else 3,
                                           (255, 255, 255), -1)
                                cv2.circle(view_bgr, pt, 5 if i in (4, 0) else 3,
                                           line, 1)
                    except Exception as e:
    ...
    def _learn_start_robot(self, idx):
        ...
        if self.port_var.get() and not self.is_sending:
            ...
            lines += [f"G1 Z0.00 F{Z_FEED}", f"G1 X0 F{f}", f"G1 Y0 F{f}"]
    ...
    def get_shape_coords(self, s):
        ...
        if s['type'] == "Line":
            ...
            # 64, not 32. The canvas draws a Circle with create_oval, so these
            # points feed G-code only — and at 32 the polygon is too coarse for
            # _fit_arcs to work with. CAL_Y makes a circle an ellipse in
            # machine space, whose curvature swings ~85% between the axis ends,
            # so a circular arc only fits near those four extremes; everywhere
            # else the fitter gave up and emitted the raw 32-gon chords, each
            # ~15mm long and sitting 0.51mm inside the true curve. Doubling the
            # samples halves the chord and quarters that error, which drops it
            # under ARC_FIT_TOLERANCE — so every run now fits an arc and no
            # straight chords are emitted at all (21 segments -> 12, all arcs).
            # Going denser than this doesn't help: the arcs already track the
            # curve, so extra points only split them into more of them.
            CIRCLE_SEGMENTS = 64
    ...
    def _paths_gcode_lines(self, paths, f, z_closed=False):
    ...
    def generate_gcode(self):
        ...
        if has_colour:
            ...
            # LOADED_COLOUR is already in the nozzle, so drawing it first costs
            # nothing; every other colour costs a trip to the refill station.
            # A design using both therefore comes out as: blue, one swap, red.
            ordered_colours.sort(
                key=lambda c: 0 if c in (LOADED_COLOUR, "Uncoloured") else 1)

            # What the nozzle holds right now. A group only triggers a swap
            # when it needs something else, so an all-blue design never stops
            # and an all-red one stops exactly once, before it starts drawing.
            in_nozzle = LOADED_COLOUR
            for colour in ordered_colours:
                ...
                if needs_switch:
                    lines += [f"G1 Z0.00 F{Z_FEED}", f"G1 X0 F{f}", f"G1 Y0 F{f}",
                              ...
                              f"G1 Z{NOZZLE_CLOSED_Z:.2f} F{Z_FEED}"]
                    if self.auto_colour_var.get():
                        # Nozzle stays closed for the trip so nothing dribbles
                        # out on the way. G4 holds the head under the dispenser;
                        # GRBL only answers "ok" once the dwell is over, which
                        # is what keeps the stream in step with the fill.
                        # No return to the origin: the first path of the group
                        # travels straight from the dock to wherever it starts.
                        # Homing first meant crossing the whole bed twice (dock
                        # -> X0 -> back out) for no reason.
                        lines += [f";AUTO_REFILL:{colour}",
                                  f"G1 X{REFILL_X:.4f} F{f}",
                                  f"G1 Y{REFILL_Y:.4f} F{f}",
                                  f"G4 P{REFILL_DWELL_S}"]
                    in_nozzle = colour
        ...
        try:
            ...
            for idx, clean in enumerate(lines):
                ...
                if clean.startswith(";COLOUR_SWITCH:"):
                    ...
                    # open and shut so the powder can't bridge and stall, and
                    # the loop checks the cancel flag every batch so Cancel can
                    # still break the operator out of the colour-change prompt.
                    if not self._purge_shake(ser, event):
                        grbl_ok = False
                        break
                    if auto:
                        # No host-side wait: the G4 dwell in the refill block
                        # below is what holds the head under the dispenser.
                        self.log_to_console(
                            f"Colour emptied — driving to the refill station "
                            f"at X{REFILL_X:g} to load {colour} "
                            f"({REFILL_DWELL_S}s fill).",
                            "info")
                    else:
                        self.log_to_console(
                    ...
                    continue
                if clean.startswith(";"):
                    # Comments mean nothing to GRBL, but sending one still
                    # costs a write plus a blocking wait for its "ok". The
                    # only comment the machine ever needed to see was
                    # ;COLOUR_SWITCH:, and that is handled above.
                    progress = (idx + 1) / total
                    self.progress_var.set(progress)
                    self.root.after(0, lambda p=progress: (
                        self.progress_bar.set(p),
