...
class ShapeApp:
    ...
    # ── Port polling ──────────────────────────────────────────────────────────
    # Ports the rangoli robot could show up as differ by OS and by which USB
    # port/cable it's plugged into, so there's no fixed device path — instead
    # rank candidates and prefer the one that actually looks like a USB
    # modem/serial device over incidental ports (e.g. macOS's built-in
    # Bluetooth virtual ports, which also show up in comports()).
    _USB_MODEM_HINTS = ("usbmodem", "usbserial", "wchusbserial", "ttyusb", "ttyacm")
    _EXCLUDE_HINTS = ("bluetooth", "debug-console")

    def _port_priority(self, device):
        d = device.lower()
        if any(h in d for h in self._EXCLUDE_HINTS):
            return -1
        if any(h in d for h in self._USB_MODEM_HINTS):
            return 2
        if d.startswith("com"):
            return 1
        return 0
    ...
    def poll_ports(self):
        ...
        if current_ports != self.last_ports:
            ...
            if current_ports:
                ...
                candidates = new or current_ports
                best = max(candidates, key=self._port_priority)
                keep_current = (self.port_var.get() in current_ports and not new)
                chosen = self.port_var.get() if keep_current else best
