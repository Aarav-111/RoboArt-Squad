...
class ShapeApp:
    ...
    def _open_learn_gallery(self):
        ...
        for key, label in (("full", "Whole design"),
        ...
        try:
            with open(LEARNER_PROFILE_FILE, "w", encoding="utf-8") as f:
                json.dump({"version":  1,
                           ...
                           "sessions": self._learn_sessions}, f, indent=2)
            return True
        except OSError as e:
            self.log_to_console(
    ...
    def _open_learn_step(self):
        ...
        popup, body = self._learn_shell(
            W, H, f"Your turn — draw this part  ·  {self._learn_level_label()}",
            f"Part {idx + 1} of {total} of '{self._learn_design}' — draw the "
            f"highlighted {col_name.lower()} part with your bottle, {after}.")
    ...
    def _open_learn_symmetry_step(self):
        ...
        W, H = S(650), S(600)
        _, body = self._learn_shell(
            W, H, f"Your turn — mirror it  ·  {self._learn_level_label()}",
            f"Symmetry {n} of {total}. The robot's {col_name.lower()} half is "
            f"solid; the dashed outline is where your "
            f"{'reflection goes' if copies == 1 else f'{copies} reflections go'}"
            f". Draw across the cyan mirror line.", outline=ACCENT_CYAN)
        ...
        tk.Label(right, text="How to mirror it:", bg=BG_CARD, fg=ACCENT_CYAN,
    ...
    def _open_learn_pulli_step(self):
        ...
        W, H = S(540), S(500)
        _, body = self._learn_shell(
            W, H, "Pulli Mode — the robot lays the dots",
            f"You've reached {self._learn_level_label()}. The robot places the "
            f"pulli for '{self._learn_design}' and nothing else — every line of "
            f"the rangoli is yours to draw, the way grandma does it.",
    ...
    def _learn_pulli_done(self, ok=True):
        ...
        self.log_to_console(
            f"Learn Mode: {len(self._learn_dots)} pulli laid — every line is "
            f"the learner's from here.", "recv")
        self._learn_advance(hint="Dots are down — the rangoli is all yours!")
    ...
    def _open_learn_camera_step(self):
        ...
        _, body = self._learn_shell(
            W, H, "Set up the rangoli camera", sub, outline=ACCENT_CYAN)
