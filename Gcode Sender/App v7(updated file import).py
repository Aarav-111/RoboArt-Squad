...
class ShapeApp:
    ...
    def _open_settings_popup(self):
        ...
        self.feed_combo = ctk.CTkComboBox(
            ...
            values=["Low", "Medium (default)", "High"], state="readonly",
            width=170, fg_color=BG_INPUT, border_color=GLASS_EDGE,
            button_color=GLASS_EDGE, button_hover_color=ACCENT_AMBER,
            text_color=TEXT_PRIMARY, dropdown_fg_color=BG_CARD,
            dropdown_text_color=TEXT_PRIMARY, font=("Segoe UI", 11))
    ...
    def import_design(self):
        ...
        path = filedialog.askopenfilename(
            ...
            filetypes=[
                ("All Supported", "*.dxf *.png *.jpg *.jpeg *.bmp"),
                ("DXF Files", "*.dxf"),
        ...
        self.log_to_console(f"Loading design: {os.path.basename(path)}", "info")

        if ext == ".dxf":
            raw_paths = self._load_dxf_paths(path)
        elif ext in (".png", ".jpg", ".jpeg", ".bmp"):
            raw_paths = self._load_image_paths(path)
        else:
            self.log_to_console(f"Unsupported file type: {ext}", "err")
            return
        ...
        if not raw_paths:
            self.log_to_console("No drawable entities found in the file.", "err")
            return

        raw_paths = self._postprocess_raw_paths(raw_paths)
        self._show_dxf_preview_popup(os.path.basename(path), raw_paths)

    def _load_dxf_paths(self, path):
        try:
            import ezdxf
    ...
    def _load_image_paths(self, path):
        try:
            ...
            messagebox.showerror("Missing Library",
                "opencv-python and numpy are required for image import.\n"
                "Run: pip install opencv-python numpy")
            return None

        try:
            # np.fromfile + imdecode avoids cv2.imread failing on non-ASCII paths
        ...
        min_perimeter = max(w, h) * 0.02
        raw_paths = []
        for c in contours:
            if cv2.arcLength(c, True) < min_perimeter:
                continue
            pts = [(float(px), float(h - py)) for [[px, py]] in c]
            if len(pts) >= 2:
                pts.append(pts[0])  # close the traced outline
                raw_paths.append(pts)

        if not raw_paths:
            self.log_to_console(
                "No traceable shapes found — try an image with clearer "
                "outlines/higher contrast.", "err")
        return raw_paths

    def _postprocess_raw_paths(self, raw_paths):
    ...
    def _dxf_preview_cancel(self):
        self._close_dxf_preview_popup()
        self.log_to_console("Import cancelled.", "info")
    ...
    def show_gallery_popup(self):
        ...
        fest_combo = ctk.CTkComboBox(
            ...
            width=150, fg_color=BG_INPUT, border_color=GLASS_EDGE,
            button_color=GLASS_EDGE, button_hover_color=ACCENT_AMBER,
            text_color=TEXT_PRIMARY, dropdown_fg_color=BG_CARD,
            dropdown_text_color=TEXT_PRIMARY, font=("Segoe UI", 10),
        ...
        diff_combo = ctk.CTkComboBox(
            ...
            width=120, fg_color=BG_INPUT, border_color=GLASS_EDGE,
            button_color=GLASS_EDGE, button_hover_color=ACCENT_PINK,
            text_color=TEXT_PRIMARY, dropdown_fg_color=BG_CARD,
            dropdown_text_color=TEXT_PRIMARY, font=("Segoe UI", 10),
            command=lambda v: refresh())
