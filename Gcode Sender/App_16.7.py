...
# Family accounts: sign-in, invitations, chat and sharing between relatives.
# Guarded exactly like OpenAI above — if these four files are not next to the
# app, every family feature disappears and the robot works as it always did.
# Drawing rangolis must never depend on an account system being importable.
try:
    import family_core
    import family_client
    import family_ui
    FAMILY_AVAILABLE = True
except Exception as _fam_err:      # noqa: BLE001 — a broken module is a
    FAMILY_AVAILABLE = False       # missing feature, never a failed launch
...
# floor. The camera step scans the USB video devices, lets the user pick one,
# shows its live picture, and remembers the choice. The photo is grabbed out of
# that same live feed, so what the user frames is exactly what gets saved.
...
CAMERA_PREVIEW_MS  = 66     # live-view frame interval (~15 fps — plenty here)
CAMERA_PREVIEW_MISSES = 30  # dead reads in a row before the live view gives up
...
# ── Learn Mode: thumbs-up hand-off ─────────────────────────────────────────
# The student's turn ends with "✓ I did it!" — but a child kneeling on the
# floor beside the kolam is nowhere near the keyboard. So the same hand-off can
# be given as a thumbs-up to the webcam: MediaPipe's hand landmarker finds the
# 21 joints, and _thumbs_up_from_landmarks decides the shape from the joint
# geometry alone.
#
# The decision runs on the *world* landmarks (metric 3D, hand-centric) rather
# than the image ones, and every test below is a ratio or an angle between the
# hand's own joints. Nothing is measured against the image's up: a thumbs-up
# held sideways, tilted, upside down, left- or right-handed, near or far, or
# angled away from the lens is the same set of numbers, so it is detected from
# any angle. The model file ships beside the app; without it (or without
# MediaPipe installed) the button simply stays the only way through.
...
GESTURE_POLL_MS      = 45    # UI-side drain of the detector thread's results
GESTURE_HOLD_FRAMES  = 4     # sticky hold frames before thumbs-up fires
GESTURE_CLEAR_FRAMES = 5     # consecutive clear frames before it can re-arm
GESTURE_MISS_TOLERANCE = 3   # miss frames allowed without wiping hold progress
GESTURE_PREVIEW_W    = 176   # self-view inside the Learn popup
GESTURE_PREVIEW_H    = 132
GESTURE_DETECT_W     = 480   # frames shrunk to this before detection (was 320)
# Detector confidence — low enough to catch a hand at desk distance / side
# angle; shape tests still reject noise.
GESTURE_DETECT_CONF  = 0.5
GESTURE_PRESENCE_CONF = 0.5
GESTURE_TRACK_CONF   = 0.5
# Shape thresholds, all scale-free (see _thumbs_up_from_landmarks).
# Softened for real kids/operators: a casual thumbs-up is rarely a tight fist,
# and the PIP often bends a little. Critical tests (thumb out) stay firm;
# folded fingers use fold OR tuck, and only a majority need to pass.
GESTURE_THUMB_STRAIGHT_DEG = 135.0   # min angle at the thumb's IP joint
GESTURE_THUMB_REACH        = 1.00    # thumb tip distance from wrist ÷ palm size
GESTURE_FINGER_CURL_DEG    = 125.0   # max angle at a folded finger's PIP joint (was 150 — open still passed)
GESTURE_FINGER_TUCK        = 0.65    # folded tip distance from its MCP ÷ palm (was 0.90 — open still passed)
GESTURE_THUMB_SPLAY_DEG    = 18.0    # min angle between thumb and finger axis
GESTURE_THUMB_FINGERS_MIN  = 4       # all 4 non-thumb fingers must look shut (was 3 — a majority let an open hand's odd finger through)
# Colour-emptied hand-off: hold up one finger (index out, other three shut) —
# same confirm as the colour-switch popup.
#
# Kept strict on purpose: a low bar used to fire on open palm (all five out)
# as well as a true point. Index must be clearly extended; middle/ring/pinky
# must all look curled; index must out-reach every other fingertip.
GESTURE_INDEX_STRAIGHT_DEG = 125.0   # min angle at the index PIP (finger out)
GESTURE_INDEX_REACH        = 0.95    # index tip distance from wrist ÷ palm
GESTURE_INDEX_EXTEND       = 0.78    # tip away from MCP (open finger ~0.9–1.2)
GESTURE_OTHER_FOLD_DEG     = 125.0   # must actually curl (was 155 — open still passed)
GESTURE_OTHER_TUCK         = 0.70    # tip near knuckle (was 1.05 — open still passed)
GESTURE_INDEX_VS_OTHER     = 1.18    # index tip must clearly out-reach each other tip
GESTURE_ONE_FINGERS_MIN    = 3       # all of middle/ring/pinky must look shut
GESTURE_ONE_HOLD_FRAMES    = 1       # fire on first clean detection frame, no hold required
GESTURE_ONE_MISS_TOLERANCE = 4       # consecutive misses before progress resets
# The 21 landmarks wired up as bones, for the skeleton the ⚙ Settings tester
# draws over the live picture: palm, then one chain per finger.
HAND_CONNECTIONS = (
...
# Countdown shown once the gesture (or the button, if it ever routes here) has
# fired, so a mis-read thumb can still be taken back.
GESTURE_COUNTDOWN_FROM = 3
GESTURE_COUNTDOWN_MS   = 1000

# ── Onboarding & Help ──────────────────────────────────────────────────────
# Two things live here: a built-in manual, and a first-run setup wizard.
#
# The manual is held as data rather than as a bundled .html/.md file on
# purpose. A judging table is exactly where a loose asset goes missing, and
# this app already ships as one file — text in the source cannot be left
# behind on the wrong USB stick. The same data renders two ways: inside the
# app (searchable, no browser needed) and, on request, as one self-contained
# static HTML page written next to the app and opened over file:// — so the
# "static HTML you can read offline" form exists without being load-bearing.
#
# Manual text is deliberately NOT run through self.tr(): translation is a
# network call, and the one promise this screen has to keep is that it works
# with no internet at all. The buttons and labels around it are translated.
SETUP_STATE_FILE = os.path.join(_APP_DIR, "setup_state.json")
MANUAL_HTML_FILE = os.path.join(_APP_DIR, "rangoli_manual.html")

# Step chips cycle through these, so step 1 is the same colour in the diagram,
# in the numbered list and on the printed page.
MANUAL_STEP_COLOURS = ["#7dd3fc", "#fbbf24", "#f472b6", "#4ade80", "#c4b5fd"]

# Each section is a short illustrated walkthrough: a one-line summary, then
# numbered steps drawn as a little chip-and-arrow diagram above the text, so
# the shape of a mode is readable before any of the words are.
MANUAL_SECTIONS = [
    {
        ...
        "steps": [
            ...
            ("Set the speed", "Settings → Speed. Super Low (default) suits "
                              "most powders. Use Aqua Low for very fine "
                              "powder that runs quickly."),
            ...
            ("Place a design", "Use Design Options on the canvas toolbar "
                               "(gallery, AI, import or pen), then press "
                               "Send to Bot in the bottom strip."),
    ...
    },
    {
        ...
        "steps": [
            ...
            ("Learn & Play", "Starts guided hand-drawing lessons straight "
                            ...
                            "→ Start)."),
            ("Design Options", "How you start a rangoli: Pre-designed "
        ...
        "tips": [
            ...
            "Design Options used to live only under Features — it is now a "
    ...
    },
    {
        ...
        "steps": [
            ("Open it", "Design Options (toolbar) → Pre-designed → "
                        "Gallery. The Design Options card also opens on its "
    ...
    },
    {
        ...
        "steps": [
            ("Open it", "Design Options (toolbar) → AI Generated."),
    ...
    },
    {
        ...
        "steps": [
            ("Open it", "Design Options (toolbar) → Import Designs."),
...
# bare score. Always on, and deliberately scoped to Learn Mode: it changes
# nothing about the machine, the canvas, the G-code or any other setting.
...
# Per-axis scale correction for the physical robot, applied only on the way out
# to G-code (to_machine) — the canvas stays a true square either way.
#
# BOTH ARE 1.0 AND SHOULD STAY THAT WAY. The axis mismatch these used to absorb
# is now corrected in firmware instead (grbl_settings.py --axis-ratio), because
# correcting it here cost more than it fixed:
#
#  * It destroyed the arc fitter's best case. The canvas->machine map is only
#    isotropic when CAL_X == CAL_Y (GRAPH_W == GRAPH_H and MAX_X == MAX_Y
#    already agree), and a circle is only a circle in machine units when the map
#    is isotropic. Under CAL_Y = 0.84 every Circle reached _paths_gcode_lines as
#    an ellipse, so _fit_full_circle could never fire and the one-G2 fast path
#    was dead code: a Circle came out as 7 chained arcs, a 3-ring preset as 28.
#    At 1.0 they are 1 and 3.
#  * It made ARC_FIT_TOLERANCE lie. That tolerance is stated in real
#    millimetres and converted through MAT_SPAN_MM, which assumes the full
#    MAX_Y span reaches the full mat. Squashing Y by 0.84 quietly loosened the
#    fit to 0.42 mm on that axis while the constant still read 0.35.
#  * It made the on-canvas ruler lie. draw_grid labels the ticks straight from
#    MAX_X/MAX_Y with no CAL applied, so the top of the canvas was labelled 4.7
#    while to_machine would only ever command 3.948 there.
#
# If the robot lays down an oval, its two axes travel different distances for
# the same commanded millimetre — GRBL's $100 (X steps/mm) and $101 (Y steps/mm)
# disagree, or a belt/wheel diameter differs between axes. Draw a Circle,
# measure the powder ring, and repair it at the source:
#
#     python3 grbl_settings.py --axis-ratio 24:20 --apply
#
# (a ring measuring 24 cm wide by 20 cm tall). That scales $101 so Y travels as
# far as X per commanded unit, and leaves the geometry the fitter sees square.
CAL_X    = 1.0
CAL_Y    = 1.0
...
# How far before the end of a stroke the nozzle gate starts shutting.
#
# The gate used to close in a G-code block of its own, which GRBL only reaches
# once the head has already stopped on the stroke's last point — so it slammed
# shut over a stationary spot and cut a smudge into the powder. Folding Z into
# the final G1 instead makes every axis in that block start and finish
# together, so the gate is paced to arrive shut exactly as the head lands.
#
# This distance is the only knob. The gate's speed falls out of it, because
# GRBL syncs the axes for us: longer coast = slower, gentler close, but a
# longer taper at the end of every stroke; too short and it is back to cutting.
# In machine units, where the whole bed is only MAX_X (4.7) across — 0.20 is
# roughly 10mm of real travel, not 20cm.
#
# Raising this also shortens the list of strokes that qualify: _coast_split
# needs a final segment 1.5x the coast to work with, so the slower the close,
# the more short tails fall back to the old instant one.
#
# Only straight final segments coast. A stroke ending in an arc falls back to
# the old instant close, because splitting an arc means re-deriving its I/J
# offsets without tripping GRBL's radius-agreement check (see the precision
# note in _paths_gcode_lines).
NOZZLE_CLOSE_COAST = 0.20

# The mirror of the coast, at the other end of the stroke. The gate used to
# snap open at a standstill before the head had moved at all, which fires a
# slug of powder straight down at the mat and bounces it everywhere. Opening
# it across the first stretch of travel instead lets the line fade in.
#
# Same units and the same trade as NOZZLE_CLOSE_COAST: longer = slower, softer
# start but a longer fade-in. Kept separate because the two ends do not
# necessarily want the same number — a start that bounces is a different
# problem from an end that cuts.
NOZZLE_OPEN_LEAD = 0.20

# While the operator empties the old colour the valve does not just sit open —
# it hammers between NOZZLE_OPEN_Z and NOZZLE_CLOSED_Z as fast as the Z axis
# will go. Held open, the powder column bridges above the gate and the nozzle
# stops draining; chattering the gate keeps knocking it loose. The head itself
# never moves — it stays parked at the origin the whole time.
#
# The feed is deliberately far above Z_FEED. Z_FEED is slow on purpose because
# a fast valve dumps a blob of powder at the start of a stroke; here dumping
# powder IS the job. GRBL clamps any feed to the axis max rate ($112), so this
# reads as "as fast as Z can move" rather than a literal 500 units/min.
#
# CYCLES is how many open/close pairs go out between syncs — see _purge_shake.
# It trades responsiveness for speed: the valve keeps flapping for at most this
# many cycles after the operator clicks.
PURGE_SHAKE_FEED   = 500
PURGE_SHAKE_CYCLES = 5
PURGE_SHAKE_DWELL  = 0.01

# Automatic multi-colour changing. After the operator empties the old colour
# at the origin, the head approaches that colour's station (see COLOUR_REFILL)
# in three sequential moves, holds still while the dispenser fills the nozzle,
# then leaves with X and Y together — no manual loading.
#
# Dwell is fixed: the nozzle always docks in the auto-refill dispenser for
# this many seconds, regardless of how much colour that group of strokes
# actually needs.
...
#
# Approach / depart order (avoids dragging across the mat at dock height):
#   1. Y up to REFILL_APPROACH_Y  (clear the bed / lift out of the dispenser)
#   2. X over to the target X     (travel at the safe height)
#   3. Y to the target Y          (drop into the dock, or down onto the draw)
# Same three-step sequence is used both into the dock and back out toward the
# first stroke after a refill.
#
# Per-colour docks are calibrated via Manual Drive. The nozzle is empty at
# job start, so even LOADED_COLOUR (the default colour) needs its own dock
# visit before the first stroke — every name in COLOUR_PALETTE has a station.

# Safe Y height used when entering or leaving an auto-refill dock. The head
# rises here first, then slides in X, then moves Y to the target.
REFILL_APPROACH_Y = 1.0

# Machine-unit (X, Y) of each auto-refill dock. Keys must match every name in
# COLOUR_PALETTE, including LOADED_COLOUR — the nozzle starts empty.
COLOUR_REFILL = {
    "Red":    (2.800, -0.100),   # first station
    "Green":  (3.100, -0.150),   # second station
    "Yellow": (3.600, -0.150),   # third station
}

# Swatch shown next to the Docking Calibration colour dropdown, so the
# selected station is unmistakable at a glance (not just dropdown text).
DOCK_SWATCH_COLOURS = {"Red": "#ef4444", "Green": "#22c55e", "Yellow": "#eab308"}

# Docking Calibration (Settings) saves here so calibrated dock positions
# survive a restart instead of reverting to the hardcoded values above.
...
# Back-compat aliases (Red station). Prefer colour_refill_xy(colour).
...
# Rest / start park — separate from the origin (0, 0). Same X (on the Y-axis);
# Y is slightly lesser so the head sits just off the bed, not on the origin.
#
# The start position is defined by the limit switches: before any job streams,
# the head seeks X until its limit is on, then Y until its limit is on, then
# that pose is locked as Start (G92). Only after both switches are on does
# drawing begin (drive to origin, work, return to Start). Colour purge still
# uses origin.
ORIGIN_X, ORIGIN_Y = 0.0, 0.0
START_X, START_Y = 0.0, -0.3

# Pre-job limit homing (send_gcode). X first until its switch hits (that axis
# is done), then a soft-reset + small pull-off + $21=1 re-arm (releasing the
# switch is required — a still-pressed switch re-trips on the very next move,
# which is what silently stopped Y from ever seeking), then Y the same way.
# G92 fires at that small offset off the Y switch. Sign matches Grbl $23=0
# (negative home).
# +1 seeks the positive end of the axis, -1 the negative one. Grbl $23=0
# (see above) means both switches sit at the NEGATIVE end on this machine —
# matches the original wiring, and matches X actually pressing its switch
# before this was (wrongly) flipped to +1. Seeking the wrong way just grinds
# the head into the far end stop, which also means Y was never reached: X
# ran out its full travel without an ALARM and _home_to_start_limits gave up
# before Y's seek ever started.
HOME_DIR_X = -1.0
HOME_DIR_Y = -1.0
# Just over one bed span — enough to reach the switch from the far corner,
# without a long grind against the end stop when a seek goes the wrong way.
HOME_SEEK_DIST = max(MAX_X, MAX_Y) * 1.25
HOME_SEEK_FEED = 120
# A G1's "ok" arrives when Grbl accepts the line, not when the move
# finishes — the seek timeout is real elapsed time instead: the physical
# travel time at HOME_SEEK_FEED, times a safety factor for acceleration
# ramps, plus a fixed pad for serial/reset overhead around it.
HOME_SEEK_TIMEOUT_FACTOR = 3.0
HOME_SEEK_TIMEOUT_PAD = 5.0
# Single retreat off a tripped switch during re-arm — not a search loop, just
# enough that the switch's own pin releases before the next axis moves.
# Without this, $X clears the ALARM flag but the pin is still physically
# pressed, and the very next G-code line re-trips the same hard limit before
# it can reach the next axis — which looked like "homing stops after X" with
# no error, since the failure showed up as an ALARM instead of a rejected
# unlock.
HOME_PULLOFF = 0.05
HOME_PULLOFF_FEED = 80

# Feed for any valve move the head is NOT travelling through — the standstill
# open/close that a stroke falls back to when its ends are too short to ramp
# across (see NOZZLE_OPEN_LEAD / NOZZLE_CLOSE_COAST), plus the colour-change
# and refill blocks. Never user-tunable: a fast Z slams the gate and fires a
# slug of powder at the mat, which is the bouncing and scatter this is set low
# to avoid. Was 150, which was still fast enough to throw powder.
#
# Not used by the ramped moves. Those carry no F of their own — they ride the
# stroke's own XY feed, and GRBL paces Z to arrive with the head.
Z_FEED = 50

# Non-drawing XY travel — reposition between strokes, approach a refill dock,
# start→origin approach, return to the start position. Always much faster than
# Settings → Speed (the draw feed). Every drawing G1/G2/G3 carries the settings
# feed explicitly, so GRBL drops back to the slow rate the moment powder is
# meant to land. GRBL still clamps this to the axis max rates ($110/$111) if
# the controller is set lower.
TRAVEL_FEED = 800

# Reposition speed used only for the two draw-adjacent moves: leaving the
# stroke-end gate-close to travel to the next stroke's start, and leaving an
# auto-refill dock toward the first stroke after a colour swap. Slower than
# TRAVEL_FEED (which still covers job start/end and entering a refill dock)
# so the head settles more gently right before powder starts flowing again.
...
REF_MARGIN_L = 40
# Bottom gutter must fit X-axis tick labels and the Start park (Y = -0.3 sits
# below the bed edge on the canvas). Too tight and the start marker clamps
# back onto the X-axis and looks wrong.
REF_MARGIN_B = 56
REF_MARGIN_T = 4
# Right gutter: keeps axis labels and any off-bed markers (e.g. refill when
# REFILL_X is past the bed edge) from being clipped off the canvas.
REF_MARGIN_R = 64
...
# The signed-in-user chip in the banner. Lifted just off the banner's black
# so it reads as a pressable surface without competing with the title.
CHIP_BG       = "#16162a"
CHIP_BG_HOVER = "#23233d"
ORIGIN_RED   = "#ff4d6d"
CANVAS_BG    = "#ffffff"

# Powders the machine can put down. Every name here must have a dock in
# COLOUR_REFILL so auto colour change knows where to drive — the nozzle
# starts empty, so even LOADED_COLOUR needs a fill before it can draw.
COLOUR_PALETTE = {
    "Red":    "#ef4444",
    "Green":  "#22c55e",
    "Yellow": "#eab308",
...
# Default colour for anything drawn without an explicit colour, and the
# colour drawn first in a job. The nozzle is empty at job start, so this
# still costs a trip to its refill dock like every other colour.
LOADED_COLOUR = "Red"
...
# NOTE ON UNITS: these are *machine* units, not millimetres. to_machine() maps
# the whole GRAPH_W-pixel work area onto MAX_X units, i.e. 170 canvas px == 1
# unit, and the mat that 4x4-unit span covers measures ~24 cm — so one unit is
# roughly 60 mm of real travel. A tolerance of "0.03" is therefore 1.8 mm on
# the mat and 5 canvas pixels, not the 0.03 mm it reads like. That mislabelling
# is what let a whole circle collapse into three arcs meeting at ~10 degree
# corners, which is the faceting seen on every round design.
#
# The tolerance is therefore stated in millimetres of real travel and
# converted, instead of being a bare machine-unit number. A hardcoded unit
# figure silently changes meaning whenever the bed size does: it was dialled
# in against MAX_X = 4, and raising the bed to 4.7 shrank every unit, which
# tightened the fit by ~15% and bought nothing but extra arcs.
MAT_SPAN_MM = 381.0                 # real width of the mat the bed maps onto (15 in)
# 0.35 mm is far finer than a powder line is wide, so the fit is invisible on
# the mat — but it is ~6x looser than the old 0.07 mm, which is what lets a
# smooth curve collapse into a few long arcs instead of a dozen short ones.
ARC_FIT_TOLERANCE_MM = 0.35
ARC_FIT_TOLERANCE = ARC_FIT_TOLERANCE_MM * MAX_X / MAT_SPAN_MM
# _fit_full_circle validates every point of the whole path at once rather than
# guessing from a sliding window, so it can afford a wider band — and the
# payoff is the largest available: one G2 for the entire shape.
ARC_FULL_CIRCLE_TOLERANCE = ARC_FIT_TOLERANCE * 3
ARC_MIN_RUN       = 4               # points collapsed before an arc is worth it
ARC_MAX_RADIUS    = 20.0            # units (~1.2 m) — bigger is effectively straight
ARC_MAX_SWEEP     = 2 * math.pi - 0.15  # stop short of a full circle (start==end is ambiguous)
ARC_CORNER_ANGLE  = math.radians(30)    # turn sharper than this is a real corner, not noise

# Generated shapes (Circle, Flower, Preset) reach the fitter as exact formula
# output, but imported and hand-drawn ones arrive with ripple: a traced kolam's
# vertices are quantised, and Tk's quadratic spline through their midpoints is
# only C1, so its curvature steps at every source vertex. That ripple is far
# below ARC_FIT_TOLERANCE and invisible on screen, yet it is enough to make the
# tangent-chained fit oscillate (see _denoise_path), so it is filtered out
# before the fitter ever sees it. Three binomial passes is a Gaussian of about
# 1.2 samples: it knocks the shortest wavelengths down ~8x while displacing a
# 5 mm-radius curve by ~0.03 mm, a tenth of the tolerance the fit already uses.
ARC_DENOISE_PASSES = 3
# Samples either side of a point used to decide whether it is a real cusp,
# for the filter's purposes only (see _denoise_anchors). At GCODE_SAMPLE_PX
# this reaches ~1.8 mm each way: past the ripple, well short of the smallest
# feature any kolam actually has.
ARC_DENOISE_WINDOW = 4
# Consecutive curvature reversals before the chain is treated as oscillating
# rather than as following real geometry. One reversal is an S-bend, which
# every design has; two in a row already means the arcs are alternating.
...
def _denoise_anchors(points, corner=ARC_CORNER_ANGLE,
    ...
    for k in range(m):
        if turn[k] <= corner:
            continue
        # Strongest turn in the neighbourhood wins; ties go to the lowest index
        # so a flat-topped cluster still resolves to exactly one anchor.
...
def _denoise_path(points, passes=ARC_DENOISE_PASSES,
    ...
    if closed:
        ...
        if not cusps:
            # Nothing to pin, so filter right around the loop. Smoothing a
            # closed curve as if it were an open one would hold the seam
            # still while its neighbours moved, i.e. build a dent at exactly
            # the point where the pen sets down.
            for _ in range(passes):
                body = [_binomial(body[(k - 1) % m], body[k], body[(k + 1) % m])
                        for k in range(m)]
            return body + [body[0]]
        # Rotating a cusp onto the seam turns the wrap-around into an ordinary
        # run of open spans, and a cusp is held fixed anyway. The rotation is
        # undone before returning: choosing where a filter starts reading is
        # not the same as choosing where the pen sets down, and _fit_arcs_
        # best_seam is what decides the latter.
        k0 = cusps[0]
    ...
    for a, b in zip(bounds, bounds[1:]):
        if b - a < 3:          # nothing between the two fixed ends to move
            continue
        span = out[a:b + 1]
        for _ in range(passes):
            span = ([span[0]]
...
def _seam_tangent(points, corner=ARC_CORNER_ANGLE):
    ...
    if math.hypot(points[0][0] - points[-1][0],
                  points[0][1] - points[-1][1]) > 1e-9:
        return None                     # open path — nothing to meet up with
    ...
    dot = max(-1.0, min(1.0, prev[0] * nxt[0] + prev[1] * nxt[1]))
    if math.acos(dot) > corner:
        return None                     # a genuine cusp sits on the seam
...
def _rotate_seam_to_corner(points, corner=ARC_CORNER_ANGLE, close_tol=1e-6):
    ...
    if math.hypot(points[0][0] - points[-1][0],
        ...
        return points                   # open path — its ends are real
    flags = _polyline_corners(points, corner)
    body = points[:-1]                  # drop the duplicated seam point
    k = next((i for i, is_corner in enumerate(flags[:len(body)]) if is_corner),
             None)
    if not k:                           # no cusp, or the seam is already on one
        return points
...
def _fit_full_circle(points, tol=ARC_FULL_CIRCLE_TOLERANCE):
    ...
    if math.hypot(points[0][0] - points[-1][0],
        ...
        return None                     # not a closed path

    # Three points spread a third apart give a well-conditioned circumcircle;
    # neighbouring samples would be nearly collinear and fit badly.
    ...
    # GRBL will draw the circle through points[0], so that is the radius the
    # rest of the path has to agree with — not the least-squares average.
    ...
    if sweep is None:
        return None                     # winding reverses somewhere
    total, code = sweep
    if abs(total - 2 * math.pi) > 0.10:
        return None                     # not exactly one lap
    return (cx, cy, code)
...
def _fit_arcs(points, tol=ARC_FIT_TOLERANCE):
    ...
    i = 0
    # Direction the previous segment left points[i] in; None means "free fit",
    # which is correct at a cusp. On a closed path the chain starts from the
    # curve's own tangent at the seam so the loop closes on itself smoothly.
    incoming = _seam_tangent(points)
    # Curvature direction of the arc just emitted, so a chain that starts
    # flip-flopping between G2 and G3 can be spotted (see _reject_oscillation).
    prev_code = None
    flip_run = 0
    while i < n - 1:
        best = None  # (end_index, cx, cy, code)
        if incoming is not None:
            # A tangent-constrained circle is pinned by only two points, so it
            # can still cover the last handful of a path that ARC_MIN_RUN would
            # otherwise hand to chorded G1 moves. Those chords cut the corner,
            # which shows up as a flat spot and a few degrees of kink right at
            # the seam of every closed curve.
        ...
        if best is not None:
            ...
            # No circle fits, so this run is straight (or too short to fit).
            # Emit ONE G1 to the far end of it rather than one per sample:
            # the samples are ~0.5 mm apart, and a G1 each is what turns a
            # straight run into hundreds of micro-moves that GRBL has to
            # decelerate into and out of. Growing stops at a corner, and
            # every skipped point still has to lie within `tol` of the chord,
            # so nothing is cut.
...
# Realistic colored-powder reference photos (one per DXF, same base filename)
# used as the gallery card's cover image instead of the plain line thumbnail.
COVER_IMAGES_DIR = os.path.join(DOWNLOADS_DIR, "Cover Images")
_DXF_STATE_RE = re.compile(r"^\d+_(.+?)_-_")

# Imported designs (DXF, traced photo, AI, notebook page) arrive already fitted
# to the bed, so unlike the generated shapes their 'size' is not a radius in
# pixels — it is a percentage of that as-imported size. 100 = exactly as it
# landed, 50 = half, 200 = double. Designs saved before the Size slider reached
# imports stored 0, which is read as this baseline.
...
# Only powders the machine actually carries, so a lesson never asks the
# student to load a colour that isn't there. Parts cycle red / green / yellow.
LEARN_PART_COLORS = [
...
class ShapeApp:
    def __init__(self, root):
        ...
        self.feed_rate            = tk.StringVar(value="Super Low (default)")
        self.port_var             = tk.StringVar()
        self.size_val             = tk.IntVar(value=50)
        self.is_moving            = False
        # Direct press-drag-release move of a stroke, no right-click/Move
        # step needed — see on_canvas_drag / on_canvas_release.
        ...
        self._nb_hub_popup        = None   # the "which notebook?" chooser
        self._nb_pages_popup      = None   # one book's pages, opened from it
        ...
        self._predesigned_dxf_files = {}  # display name -> filename, filled by gallery load
        self._cover_image_cache  = {}     # (filename, px) -> CTkImage, or None if missing
        ...
        # Automatic colour changing is the normal way to run now: the operator
        # only confirms the old colour is emptied, and the refill itself is
        # done by the dispenser at REFILL_X. Turn it off to go back to loading
        # the next colour into the nozzle by hand at the origin.
        self.auto_colour_var      = tk.BooleanVar(value=True)
        # Settings → Docking Calibration popup.
        self._dock_calib_dlg = None
        self._dock_check_var = None
        # Colour the nozzle is actually parked at in the calibration popup,
        # per Drive to dock — cleared on colour switch so Save can't write
        # one colour's real position under a different colour's name.
        ...
        # Thumbs-up hand-off. The camera read and the landmark detection both
        # block, so they live on a worker thread; the UI only ever touches
        # _gesture_latest (last frame + verdict) and the after-loop below.
        self._gesture_thread   = None
        self._gesture_stop     = None   # threading.Event the worker watches
        self._gesture_after    = None   # after-id of the UI-side poll
        self._gesture_latest   = None   # (rgb frame, is_thumbs_up) from worker
        self._gesture_lbl      = None   # Tk label the self-view lands on
        self._gesture_hint     = None   # "show a thumbs-up" caption
        self._gesture_img      = None   # PhotoImage reused per frame
        self._gesture_run      = 0      # sticky thumbs-up hold progress
        self._gesture_miss     = 0      # consecutive miss frames
        self._gesture_clear    = GESTURE_CLEAR_FRAMES   # consecutive clear ones
        self._gesture_armed    = True   # False after a fire, until hand clears
        self._gesture_error    = None   # why the watcher isn't running, if so
        self._countdown_popup  = None
        self._countdown_after  = None
        self._gesture_test_dlg = None   # ⚙ Settings → Gesture control test
        # Colour-emptied one-finger watcher (camera + hold index out).
        self._empty_gesture_thread = None
        self._empty_gesture_stop   = None
        self._empty_gesture_after  = None
        self._empty_gesture_latest = None  # bool: one finger out this frame
        self._empty_gesture_run    = 0     # sticky hold progress
        self._empty_gesture_miss   = 0     # consecutive miss frames
        self._empty_gesture_error  = None
        self._empty_gesture_lbl    = None  # optional status line on the popup
        # Manual Drive (⚙ Settings → Manual Drive): a held-open serial session
        # so the jog pad can fire moves without reconnecting every click.
        self._manual_drive_dlg = None
        self._manual_ser       = None   # serial.Serial while the pad is open
        self._manual_busy      = False  # True while a jog/go-to is in flight
        self._manual_pos       = None   # (x, y) machine units, if known
        self._manual_step_var  = None   # StringVar for step size, set by dialog
        ...
        # ── Kid Mode ────────────────────────────────────────────────────────
        # The cartoon skin over Learn Mode is permanent, and so are its beeps
        # — there is no switch for either any more. The attributes stay
        # because the k_* theme accessors, the mascot/confetti code and
        # _kid_sound all read them.
        self.kid_mode      = True
        ...
        # pages are already the right way up. See _learn_tf_for.
        ...
        # ── Learn Mode: USB camera — scan, pick, watch live, photograph
        ...
        # The live view. Only the camera step builds the widgets, so everywhere
        # else these stay None and the code falls back to a single test frame.
        self._camera_view_lbl   = None    # label the frames are blitted into
        self._camera_view_size  = (0, 0)  # blit size, fixed by the popup layout
        self._camera_view_cap   = None    # VideoCapture held open by the view
        self._camera_view_index = None    # device the view is showing
        self._camera_view_after = None    # after-id of the frame loop
        self._camera_view_img   = None    # Tk drops the image without a ref
        self._camera_view_misses = 0      # consecutive failed reads
        self._camera_view_opening = False # an open-on-worker-thread is in flight
        self._camera_view_token = 0       # bumped per open; stale ones are dropped
        self._camera_last_frame = None    # newest live frame — what Take saves
        ...
        # ── Onboarding & Help ───────────────────────────────────────────────
        self._help_popup    = None    # the manual
        self._help_section  = MANUAL_SECTIONS[0]["id"]
        self._help_query    = ""      # live search box contents
        self._help_entry    = None
        self._setup_popup   = None    # first-run wizard
        self._setup_step    = 0

        # ── Family accounts ─────────────────────────────────────────────────
        # The client opens the local replica immediately and needs no network
        # to do it, so this is safe to construct before the UI exists. Whoever
        # was signed in last is restored here, offline.
        self.family      = None      # FamilyClient, or None if unavailable
        self._family_hub = None      # the open hub popup, if any
        # ── Competitions ────────────────────────────────────────────────
        # The competition every finished rangoli is entered into, remembered
        # in the learner profile so it survives a restart. None means the
        # child is practising alone, which is the default.
        self._competition_hub = None
        self._active_competition_id = None
        self._topmost_yielded = []   # popups that stood down for the hub
        self._family_badge_after = None   # the chip's repeating timer
        self._init_family()
    ...
    def _color_button(self, parent, text, command, color, *,
                      ...
                      text_color="#ffffff", bg_color=None):
    ...
    def _build_canvas_overlays(self, host):
        ...
        # Top-right: Features / Learn & Play / Design Options / AI Enhance
        top_right = _panel(relx=1.0, rely=0.0, anchor="ne", x=-6, y=6)

        # Features holds teaching extras and Simulate. Design Options sits on
        # the toolbar (was Simulate's slot) so starting a rangoli is one click.
        # Learn Mode is the thing the app is for, so it gets its own place
        # on the toolbar as well as its row inside Features.
        ...
        # Learn Mode, straight from the canvas. Same toggle as the row in
        # the Features popup, so the button's label follows the mode.
        self.learn_toolbar_btn = self._color_button(
            top_right, "\U0001f393 Learn & Play", self._toggle_learn_mode,
            "#0f766e", width=S(150), height=S(40), font_size=FS(12))
        self.learn_toolbar_btn.pack(side="left", padx=(S(0), S(8)))
        self.register_translatable(self.learn_toolbar_btn, "Learn & Play",
                                    fmt=lambda t: f"\U0001f393 {t}")

        # Design Options — gallery / AI / import / pen. Used to live under
        # Features; now here in place of Simulate (Simulate moved into Features).
        self.design_options_btn = self._color_button(
            top_right, "Design Options", self._open_design_options_popup,
            ACCENT_CYAN, width=S(150), height=S(40), font_size=FS(12))
        self.design_options_btn.pack(side="left", padx=(S(0), S(8)))
        self.register_translatable(self.design_options_btn, "Design Options")
        ...
        self.register_translatable(self.ai_fx_btn, "AI Enhance",
                                    fmt=lambda t: f"\u2728 {t}")
        self.simulate_btn = None   # lives in the Features popup while open
        self._sim_running = False
    ...
    def _build_banner(self, parent):
        ...
        # The manual sits in the banner, not behind Settings: someone who does
        # not know what any of this does should not have to guess which menu
        # the explanation is under.
        self.help_btn = self._color_button(
            controls, "❓ Help", self._open_help_manual, "#1e293b",
            width=S(84), height=S(36), font_size=FS(11), corner_radius=S(10))
        self.help_btn.pack(side="left")
        self.register_translatable(self.help_btn, "Help",
                                    fmt=lambda t: f"❓ {t}")

        # Family lives in the banner rather than behind Settings: an
        # invitation waiting for an answer is somebody else being kept
        # waiting, and it has to be visible without going looking for it.
        # It shows who is signed in — a face and a name say "this is your
        # account, tap it" far more directly than a labelled button, and it
        # doubles as the answer to "whose rangolis are these?".
        self.family_chip = None
        if self.family is not None:
            self._build_family_chip(controls)
            # A two-line chip is taller than the banner's text-derived
            # height on smaller screens, and pack_propagate(False) would
            # simply crop it. Grow the strip to whatever it actually needs
            # rather than guessing a constant that holds on one laptop.
    ...
    def _set_language(self, code):
        ...
        # The chip builds its own text from self.tr() rather than through
        # register_translatable — a person's name must not be translated,
        # only the signed-out prompt around it.
    ...
    def _open_settings_popup(self):
        ...
        self.root.update_idletasks()
        # Clamped to the screen so the card can't run off the bottom on a
        # small display. Tall enough for Connection / Speed / Manual Drive /
        # Robot Test / Gesture control without crowding.
        W = S(400)
        ...
        self.feed_combo = ctk.CTkComboBox(
            ...
            values=["Aqua Low", "Super Low (default)", "Low", "Medium", "High"],
            state="readonly",
            width=S(170), fg_color=BG_INPUT, border_color=GLASS_EDGE,
            button_color=GLASS_EDGE, button_hover_color=ACCENT_AMBER,
            text_color=TEXT_PRIMARY, dropdown_fg_color=BG_CARD,
        ...
        # Auto colour change lives on Manual Drive (next to the refill park
        # control), not here — Settings stays connection/speed/test only.
        ...
        # 3a) Docking Calibration — a single popup that drives to any
        # colour's auto-refill dock (via the normal safe sequential path)
        # and lets you jog + save a new position from there.
        slot = _row("Docking Calibration",
                    "Drive to a colour's dock and fine-tune or save its "
                    "position.")
        self._color_button(slot, "\U0001f3af Open", self._open_dock_calibration_dialog,
                           ACCENT_AMBER, width=S(170), height=S(30),
                           font_size=FS(11)).pack(side="right")

        # 3b) Manual Drive — jog the head or send it to absolute coordinates
        # without placing a design. Opens its own pad so the D-pad has room.
        slot = _row("Manual Drive",
                    "Jog left/right/up/down, or type X/Y and drive there. "
                    "Nozzle stays closed — for positioning only.")
        self._color_button(slot, "\U0001f3ae Open pad",
                           self._open_manual_drive_dialog, ACCENT_CYAN,
                           width=S(170), height=S(30),
                           font_size=FS(11)).pack(side="right")

        # 4) Bench camera gestures without a full lesson / print: thumbs-up
        # (Learn Mode hand-off) and one-finger-out (colour emptied).
        ...
        self._color_button(slot, "\U0001f590 Open tester",
                           self._open_gesture_test_dialog, ACCENT_GREEN,
                           width=S(170), height=S(30),
                           font_size=FS(11)).pack(side="right")

        # Kid Mode and its sounds are both always on now — the cartoon skin is
        # simply how the app looks, so neither has a switch here any more.
    ...
    # ── Manual Drive (Settings → Manual Drive) ───────────────────────────────
    # A held-open serial session so the D-pad and go-to fields can fire short
    # G-code moves without reconnecting for every click. The nozzle is always
    # left closed — this is for parking the head, not for drawing powder.

    def _manual_feed(self):
        _SPEED_MAP = {
            "Aqua Low": 50, "Super Low (default)": 100,
    ...
    def _manual_drive_line(self, ser, line):
        ...
        while True:
            ...
            if "ok" in low:
                ...
                return True
            # Banners / status chatter — keep waiting for ok.
    ...
    def _manual_query_pos(self, ser):
        ...
        while time.time() < deadline:
            ...
            if not res:
                continue
            # <Idle|MPos:1.000,2.000,0.000|FS:0,0>  (or WPos on some builds)
            m = None
    ...
    def _manual_drive_connect(self):
        ...
        try:
            ...
            # Mark the port in use so Send cannot steal it mid-jog.
    ...
    def _manual_drive_disconnect(self):
        ...
        # Only clear is_sending if we were the ones holding it for the pad.
        # A print job never shares this path (we refuse to open while busy).
            ...
            try:
                ...
                for line in lines:
                    if not self._manual_drive_line(ser, line):
                        ...
                        # Drop a dead handle so the next click reconnects.
                        try: ser.close()
                        except Exception: pass
                        self._manual_ser = None
                        self.is_sending = False
                ...
                # Prefer GRBL's own report when it answers.
                pos = self._manual_query_pos(ser)
                if pos is not None:
                    self._manual_pos = pos
    ...
    def _manual_jog(self, axis, sign, status_cb=None):
        ...
        delta = sign * step
        f = self._manual_feed()
        # Relative move, then restore absolute so a later print job is safe.
        lines = [
            "G91",
    ...
    def _open_dock_calibration_dialog(self):
        ...
        # ── Colour picker + drive ────────────────────────────────────────
        # Always starts on the first colour, and any station switch clears
        # "driven to" tracking below — stale selections can no longer save
        # a position under the wrong colour's name.
        ...
        self._color_button(body, self.tr("\U0001f9ed Drive to dock"),
                           drive_to_dock, ACCENT_CYAN,
                           width=W - 40, height=S(34), font_size=FS(11)
                           ).pack(pady=(S(0), S(10)))

        # ── D-pad, fine step for dock alignment ─────────────────────────
        ...
        self._color_button(body, self.tr("\U0001f4be Save as new position"),
    ...
    def _open_manual_drive_dialog(self):
        ...
        W = S(420)
        # Tall enough for the D-pad + go-to + Connect/Close without clipping.
        ...
        # Footer first so Connect/Close always keep their row at the bottom.
        ...
        # Live position (left) + automatic colour loading (right) — same row
        # as the green X/Y readout so the switch sits at that coordinate line.
        ...
        step_combo = ctk.CTkComboBox(
            ...
            dropdown_text_color=TEXT_PRIMARY, font=("Segoe UI", FS(11)))
        step_combo.pack(side="right")
        # Allow typing a custom step as well as picking a preset.
        try: step_combo.configure(state="normal")
        except Exception: pass
        ...
        # ── Axis nudge chips (same as D-pad, clearer for x/y wording) ──────
        ...
        # Quick presets for common parks
        ...
        # Keyboard: arrows / WASD jog, H homes, Esc closes. Ignored while the
        # focus is inside a text field so typing coordinates stays normal.
        def on_key(event):
    ...
    # ── ONBOARDING & HELP: the built-in manual ───────────────────────────────
    # Offline by construction: the text is MANUAL_SECTIONS above, the diagrams
    # are drawn with canvas primitives, and nothing here opens a socket.
    ...
    def _manual_draw_steps(self, canvas, titles, width, height):
        ...
        for i, title in enumerate(titles):
            ...
            # The chip carries the step's own name, wrapped — a numbered box
            # with no word in it explains nothing.
    ...
    def _open_help_manual(self, section_id=None):
        ...
        # No "Close" button here: the ✕ at the top-right already closes this.
    ...
    def _render_manual_index(self, side, width):
        ...
        # Keep the reader on a section that is still on screen after a search.
    ...
    def _help_show_section(self, section_id):
        ...
        if popup is None:
            return
        # Rebuilding both panes is the honest way to keep the index highlight
        # and the content in step; the manual is far too small for this to be
        # worth optimising.
    ...
    # ── the same manual as one static offline HTML page ──────────────────────
    ...
    # ── ONBOARDING: the once-only first-run setup wizard ─────────────────────
    # Port, speed, camera — the three things that have to be right before the
    # robot does anything, asked once and then never again, so day-to-day use
    # is not cluttered by setup questions.

    # ── Family accounts ───────────────────────────────────────────────────────
    # Sign-in, invitations, chat and sharing between relatives. The heavy
    # lifting is in family_core / family_client / family_ui; what lives here
    # is only the wiring, so this file stays about the robot.
    ...
    def _init_family(self):
        ...
        try:
            ...
            # The client logs from its background sync thread, so every line
            # is bounced onto the UI thread — and dropped without a traceback
            # if the window has already gone.
    ...
    def _open_family_hub(self, tab="family"):
        ...
        if not self.family.is_logged_in():
            # Not signed in: send them through the gate first, then land on
            # the tab they actually asked for.
            def after_gate():
                self._refresh_family_chip()
    ...
    def _open_competition_hub(self, tab="compete"):
        ...
        if self._competition_hub is not None:
            try:
                ...
                self._competition_hub = None
        # The Learn gallery this is opened from is itself "-topmost". Two
        # topmost overrideredirect windows have no defined order on macOS, so
        # the hub lands behind the gallery about half the time. Drop the
        # gallery's claim for as long as the hub is up, and put it back after.
        self._yield_topmost()
        self._competition_hub = family_ui.CompetitionHub(self, tab=tab)
        self._raise_above_popups(self._competition_hub.win)

    # Popups that assert "-topmost" and would otherwise fight a window opened
    # on top of them. Any that is not currently open is simply skipped.
    ...
    def _family_logout(self):
        ...
        def done():
            ...
            self._refresh_family_chip()
            # Straight back to the gate: the app is now nobody's, and the
            # next person should not inherit the last one's session.
    ...
    def _build_family_chip(self, parent):
        ...
        # Invitation count. Packed only when there is one, so the chip keeps
        # its natural width the rest of the time.
        self._chip_badge = tk.Label(
            pad, text="", bg=ACCENT_AMBER, fg="#000000",
            font=("Segoe UI", FS(9), "bold"), padx=S(5))

        # The whole chip is the hit target, not just whichever label the
    ...
    def _refresh_family_chip(self):
        ...
        try:
            if self.family.is_logged_in():
                ...
                # Falls back to the account's own contact so the second line
                # is never blank before they have a family.
        ...
        # One timer chain only. This is called directly on sign-in, sign-out
        # and from the gate, and without cancelling first each of those would
        # start another loop that never stops.
    ...
    def _boot_sequence(self):
        ...
        # Identity first, so whatever gets drawn is attributed from the very
        # first stroke rather than being reassigned afterwards.
    ...
    def _boot_after_signin(self):
        ...
        # An invitation waiting for you is the one thing worth interrupting
        # the morning's page for — it is somebody else waiting on an answer.
    ...
    def _resume_after_setup(self):
        ...
        # The morning's page from her notebook, once per day, on top of the
        # design chooser. Silent if the notebook is empty or today's page has
    ...
    def _open_setup_wizard(self, first_run=True, step=0):
        ...
        # Step pips, so it is obvious this is short.
        ...
        if step == 0:
            ...
            _head(self.tr("How fast should it draw?"),
                  self.tr("This is the feed rate used when the design is "
                          "streamed to the robot. Super Low (default) suits "
                          "most powders. Pick Aqua Low for very fine powder that "
                          "runs quickly, and Medium or High only once you "
                          "know the powder flows cleanly."))
            ctk.CTkComboBox(
                body, variable=self.feed_rate,
                values=["Aqua Low", "Super Low (default)", "Low", "Medium",
                        "High"],
                state="readonly", width=S(230), fg_color=BG_INPUT,
                border_color=GLASS_EDGE, button_color=GLASS_EDGE,
                button_hover_color=ACCENT_AMBER, text_color=TEXT_PRIMARY,
            ...
            _head(self.tr("Is the camera working?"),
                  self.tr("A USB camera over the mat photographs the finished "
                          ...
                          "without one."))
            # Reuse the camera install screen's own widgets and handlers, so
            # there is exactly one implementation of scanning and installing.
            ...
            self._color_button(
                body, self.tr("\U0001f50d Scan for USB cameras"),
    ...
    def _finish_setup(self, skipped):
        ...
        self._close_setup_wizard()
        # Marked done either way: a wizard that reappears because it was
        # skipped is a wizard that gets skipped harder next time. ❓ Help →
        # Setup wizard re-runs it on demand.
    ...
    def _close_setup_wizard(self):
        ...
        # These point at wizard widgets while it is open; the camera helpers
        # check winfo_exists, but clearing them is tidier than relying on it.
    ...
    def _open_features_popup(self, force=False):
        ...
        extras. Design Options lives on the canvas toolbar now.
        ...
        if self._features_popup is not None and force:
            ...
            return
        self.root.update_idletasks()
        # Learn (carrying Progress), Simulate, Picture to Rangoli, Kolam
        # Notebook, Kolam of the Day. Capped to the screen so the last row
        # is never cut off on a short laptop display — the body scrolls.
        ...
        # Scrollable, because the list can outgrow a short laptop screen: the
        # last row and its description would otherwise run off the bottom edge
        # and make that feature look as though it did not exist.
        ...
        self.learn_btn = self._small_btn(
            ...
            width=S(96))
        # Progress & Impact sits beside Start instead of owning the last row of
        # the popup: it reports on the lessons, so it belongs with them, and it
        # no longer needs the reader to scroll to the bottom to find it.
        self._small_btn(
        ...
        # Simulate — preview the robot toolpath on the canvas. Lives here
        # now; Design Options took its old toolbar slot. Not _small_btn:
        # that fires once, and Start/Stop on the same row needs both clicks.
        ...
        # Import from Photo was removed from this popup. _launch_photo_import
        # is kept in place but is no longer wired to any button.
        ...
        # Progress & Impact no longer has a row here — its button moved up
        # beside Learn Mode's Start.
    ...
    def _launch_design_options(self):
        """Open Design Options (toolbar or legacy callers). Close Features
    ...
    def setup_context_menu(self):
        # Do NOT use tk.Menu / .post() / .tk_popup() on macOS.
        # On recent macOS (esp. Tahoe / 26.x) Tk's native NSMenu path aborts
        # the whole process inside objc_autoreleasePoolPop (SIGABRT,
        # AutoreleasePoolPage::busted). A plain Toplevel matches the rest of
        # the app's popups and never touches NSMenu.
    ...
    def _show_context_menu(self, x_root, y_root):
        ...
        # Click-away and Escape dismiss without taking an action.
        popup.bind("<Escape>", self._close_context_menu)
        try:
            popup.grab_set()
        except tk.TclError:
            pass
        # Outside click: grab routes presses here only while open; bind
        # Button-1 on the shell so empty chrome closes the menu.
        glass.bind("<Button-1>", self._close_context_menu)
        popup.focus_force()

    # ── Port polling ──────────────────────────────────────────────────────────
    # The robot enumerates as a USB-serial bridge — "usbmodem" for a native-USB
    # board, "usbserial"/"wchusbserial"/"SLAB_USBtoUART" for the common CH340
    # and CP210x adapters, ttyACM/ttyUSB on Linux, COMn on Windows. Ranking by
    # name matters because macOS also exposes built-in ports that are never a
    # robot (Bluetooth-Incoming-Port, wlan-debug, debug-console); picking the
    # first device alphabetically used to land on one of those.
    _PORT_HINTS = (
        (100, ("usbmodem",)),
    ...
    # Never auto-selected. Still listed in the dropdown for manual override.
        ...
        if current_ports != self.last_ports:
            if self.port_combo is not None:
                try:
                    ...
                    pass
            if current_ports:
                # A port that just appeared is almost certainly the robot being
                # plugged in, so prefer the best of those; otherwise keep the
                # current pick if it's still present and plausible, and only
                # fall back to a fresh scan when it isn't.
    ...
    def from_machine(self, mx, my):
        # Exact inverse of to_machine, so round-tripping the predesigned G-code
        # back onto the canvas lands where it was drawn.
    ...
    def load_and_send_rangoli(self, event=None):
        ...
        self.redraw()
        # Same Start → Origin → work → Start flow as generate_gcode.
    ...
    def _draw_refill_dock(self, c, x0, x1, y1):
        ...
        for name, (rx, ry) in COLOUR_REFILL.items():
            ...
            # Darker outline for contrast on the white canvas.
            outline = {
                "Green":  "#14532d",
                "Red":    "#7f1d1d",
                "Yellow": "#854d0e",
    ...
    def _draw_start_position(self, c, x0, y1):
        ...
        """
        ox, oy = x0, y1  # origin corner on the plot
        # True machine → canvas map (same scale as refill docks).
        px = x0 + (START_X / (MAX_X * CAL_X)) * GRAPH_W
        py_true = y1 - (START_Y / (MAX_Y * CAL_Y)) * GRAPH_H

        d = S(9)
        # Keep X on-canvas; start is at X=0 so this is just the origin column.
        px = min(max(px, d + S(4)), CANVAS_W - d - S(4))

        if START_Y < 0:
            # Below the X-axis. Clear the origin rings (~S(22)) and stay in /
            # near the bottom margin so the park is visibly under Origin.
            gap = max(S(30), min(abs(py_true - y1), MARGIN_B - S(4)))
            # If the bottom margin is too tight, still push past the axis.
            gap = max(gap, S(30))
            py = y1 + gap
        elif START_Y > 0:
        ...
        # Soft clamp only so the diamond itself isn't fully off-widget.
        py = min(max(py, d + S(2)), CANVAS_H - d - S(2))

        fill = ACCENT_CYAN
        outline = "#0e7490"

        # Dashed lead from origin → start so the offset is readable.
        if abs(START_X) > 1e-6 or abs(START_Y) > 1e-6:
            c.create_line(ox, oy, px, py, fill=fill, width=S(2),
                          dash=(4, 3), tags="grid")

        # Diamond park marker (rotated square).
        c.create_polygon(
            px, py - d, px + d, py, px, py + d, px - d, py,
            fill=fill, outline=outline, width=S(2), tags="grid")
        # Inner white pip so it reads as a rest-park, not a refill dock.
        ir = S(3)
        c.create_oval(px - ir, py - ir, px + ir, py + ir,
                      fill="#ffffff", outline="", tags="grid")
        # Labels to the right of the diamond (under the bed, not on the axis).
    ...
    def draw_grid(self):
        ...
        # MAX_* may be fractional (e.g. 4.5), so tick loops walk whole units
        # up to the bed size and simply stop short of a partial last unit.
        ...
        c.create_oval(ox-rd, oy-rd, ox+rd, oy+rd, fill="#ffffff", outline="",
                      tags="grid")
        # Label origin so it stays distinct from the cyan START park nearby.
    ...
    # A DXF colour within this many degrees of hue from a palette colour is
    # treated as "the same colour, just lighter/darker" and snapped to it
    # silently. Further than this is a genuinely different colour — flagged
    # as incompatible and forced to Red instead of guessed.
    ...
    def _dxf_entity_colour_name(entity, doc, ezdxf_colors):
        ...
        if true_color is not None:
            ...
            if aci in (0, 256):   # BYBLOCK / BYLAYER — resolve via the layer
            ...
            if aci == 7 or not (1 <= aci <= 255):
                return None       # default colour — no explicit intent
        ...
        if s < 0.15:
            return None            # near grey/white/black — no colour intent
    ...
    def _parse_dxf_file(self, path):
        ...
        if not raw_paths:
            ...
            # ignoring the missing subclass structure entirely. No per-entity
            # colour is available from this path.
        ...
        def _chain(pieces):
            ...
            while remaining:
                ...
                # Reverse in place rather than list(reversed(nxt)) — a new
                # list would carry a new id(), breaking colour_by_id's
                # link to this piece for every path that gets flipped here.
                if best_rev:
                    nxt.reverse()
                ordered.append(nxt)
            return ordered
    ...
    def _show_dxf_preview_popup(self, filename, raw_paths, dxf_colours=None):
        ...
        # Seeded from the DXF's own entity colours, if any were found — the
        # operator can still repaint or clear any stroke before confirming.
        state = {
    ...
    def _open_notebook_hub(self):
        ...
        # Bottom strip first, so the scrolling list can be sized around it.
        foot = tk.Frame(popup, bg=BG_CARD)
        ...
        for title in sorted(books):
            ...
            # _color_button rather than _small_btn: the latter wraps itself in
            # a BG_CARD frame, which would show as a patch on these BG_INPUT
            # rows.
            self._color_button(
    ...
    def _open_notebook_pages(self, book):
        ...
        for idx, (name, _full, data) in enumerate(pages):
            ...
            if self._saved_guide_dots(data):
                pul = tk.Label(card, text="✋", bg=BG_INPUT, fg=ACCENT_CYAN,
                               font=("Segoe UI", FS(10), "bold"),
                               cursor="hand2")
    ...
    def _open_notebook_review_popup(self, frame, result, opts):
        ...
        # Sharing a page sends the same JSON the digitizer saves locally, so
        # a relative can place it on their own canvas exactly as drawn.
        if self.family is not None:
            _foot_btn(4, "\U0001f465 Share",
    ...
    def _arm_colour_emptied_button(self, event, colour="", first_load=False):
        ...
        # Same confirmation can come from holding one finger out to the
        # webcam — start watching as soon as the prompt is up.
    ...
    def _show_colour_switch_popup(self, colour, first_load=False):
        ...
        # Gesture alternative to the click: point one index finger at the
        # camera. Status is updated by _empty_gesture_poll — turns bright
        # green and says ACTIVE while the pose is locked.
    ...
    # ── Colour-emptied: one-finger-out gesture ─────────────────────────────
    def _empty_gesture_start_watch(self):
        ...
        if self._empty_gesture_thread is not None:
            return
        # Learn Mode's thumbs-up watcher owns the camera while a student turn
        # is up; don't fight it — the click path still works.
    ...
    def _empty_gesture_worker(self, stop_evt):
        ...
        try:
            ...
            if cap is None:
                ...
                return
            # Drop a couple of frames so auto-exposure settles before detect.
            for _ in range(3):
                cap.read()
        ...
        if latest is not None:
            ...
            if latest:
                ...
                if held >= n:
                    ...
                    return
                # Active hold — bright, unmistakable feedback.
                self._empty_gesture_set_status(
                    f"\U0001f446 ACTIVE gesture \u2014 keep holding "
                    f"{held}/{n}",
                    ACCENT_GREEN)
            else:
                # Tolerate brief misses so a flicker does not restart at 0.
    ...
    def _load_predesigned_dxf_library(self, state_filter=None):
        """Pull every .dxf fresh from Downloads/Predesigned Library, grouped by
        state (catalog files are named 'NNN_State_-_Style_Design_N.dxf'; the
        original hand-picked files with no state prefix land in 'Classics').
        Returns {state: {short_name: (full_name, raw_paths)}}; ``state_filter``
        (if not None/"All") restricts to a single state's group."""
        if not os.path.isdir(DOWNLOADS_DIR):
            return {}
        library = {}
        ...
        for filename in filenames:
            ...
            raw_paths, _colour_by_id, _incompat, err = self._parse_dxf_file(full_path)
            if err:
            ...
            library.setdefault(state, {})[short_name] = (full_name, raw_paths)
        return library
    ...
    def show_gallery_popup(self):
        ...
        # State filter bar — only meaningful for the pre-designed catalog.
        ...
        def _on_state_change(choice):
            # CTkComboBox command — never use tk.OptionMenu here: its native
            # NSMenu path aborts the process on recent macOS (same bug as the
        ...
        if not has_library and not saved and not notebook:
            ...
            # Pre-designed catalog, grouped under a heading per state
            # ("Classics" last — the original hand-picked files with no
            # state prefix). Single-state filter still shows its heading so
            # the active filter stays visible.
            ...
            for state in state_order:
                ...
                for idx, (short_name, (full_name, raw_paths)) in enumerate(entries):
                    r, c = divmod(idx, cols)
                    ...
                    def _draw(t, rp=raw_paths, cv=cover):
                        if cv is not None:
                            ...
                            t.image = cv  # keep a reference alive
                        else:
                            self._draw_dxf_thumbnail(t, rp, 70, 70, 56)

                    _make_card(
                        row + r, c, short_name, _draw,
                        lambda nm=full_name, rp=raw_paths:
            ...
            if notebook:
                # One sub-section per book, pages in order.
                books = {}
    ...
    def _choose_dxf_design(self, name, raw_paths):
        ...
        self._close_gallery_popup()
        self._finalize_dxf_import(self._predesigned_dxf_files.get(name, name), raw_paths)
    ...
    def _refresh_learn_toolbar_btn(self):
        ...
        on = self.learn_mode_var.get()
        try:
            btn.configure(text=("\U0001f393 " +
                                self.tr("Stop Learn & Play" if on else "Learn & Play")))
        except Exception:                          # noqa: BLE001
    ...
    def _toggle_learn_mode(self):
        ...
        # The intro plays every time Learn Mode is started, not just once per
        # app run: a child who comes back to the lesson after lunch should get
        # the bottle demonstration again, and the green button beside Pause is
        # there to skip straight past it.
    ...
    def _open_learn_gallery(self):
        ...
        # Replay the bottle demonstration without leaving and re-entering Learn
        # Mode — the only other way back to it is buried in a lesson screen.
        # Its own row: in Kid Mode the lesson chips and the sticker button
        # already fill the picker, and a fifth widget there gets squeezed out.
        ...
        vb = tk.Label(vidrow,
                      ...
                      font=("Segoe UI", FS(9), "bold"), cursor="hand2")
        vb.pack(side="right")
        vb.bind("<Button-1>", lambda e: self._show_learn_intro())

        # Competing belongs on the design-picker screen. Full buttons in the
        # popup's top-right (left of ✕) so they stay obvious and don't crowd
        # the lesson chips below. The hub opens over this gallery and hands
        # it back on close, so a look at the ranking never costs the design
        # they were about to pick.
        race_w, world_w, btn_h, btn_gap = S(158), S(168), S(40), S(8)
        close_x = W - S(42)
        world_x = close_x - btn_gap - world_w
        race_x = world_x - btn_gap - race_w
        btn_y = S(14)
        # Match the Learn card (cream in Kid Mode) so CTk's rectangular shell
        # does not show a black frame around the rounded pill.
        card_bg = self.k_card()
        race_btn = self._color_button(
            popup,
            self.kid_pick("\U0001f3c6 Race a friend", "\U0001f3c6 Compete"),
        ...
        world_btn = self._color_button(
            popup,
            self.kid_pick("\U0001f30d Who is winning?",
                          "\U0001f30d World ranking"),
    ...
    # ── Learning a page out of the notebook ──────────────────────────────────
    ...
    # ── Kid Mode: theme, skin ────────────────────────────────────────────────
    # Nothing about Kid Mode is configurable any more — the skin and its sounds
    # are both always on — so there is no config file to read or write.
        ...
        for w in kids:
            ...
            if isinstance(w, ctk.CTkBaseClass):
                if isinstance(w, ctk.CTkButton):
                    try:
                        # bg_color matches the cream card so rounded pills do
                        # not sit inside a black rectangle on Kid Mode screens.
    ...
    def _kid_build_mascot(self, parent, mood="idle"):
        ...
        self._kid_mascot_mood = mood
        self._kid_mascot_phase = 0
        # Plain speech text — not a chip/button. The lesson-type chips below
        # already look like controls; this line should read as caption only.
        self._kid_bubble = tk.Label(
            holder, text="", bg=KID_THEME["card"], fg=KID_THEME["ink"],
            font=self.k_font(FS(11)), padx=S(2), pady=S(2),
            justify="left", wraplength=S(260),
        ...
        try:
            with open(LEARNER_PROFILE_FILE, "w", encoding="utf-8") as f:
                json.dump({"version":  1,
                           ...
                           # which competition finished rangolis go to
    ...
    def _record_learn_session(self, verdict, scored_by):
        ...
        self._save_learner_profile()
        # Publish the same row to the family, scored_by and all, so relatives
        # see the practice as it happens and see how it was scored.
        self._publish_learn_session(self._learn_sessions[-1])
        # …and onto the competition board, if the child is in one.
    ...
    def _open_learn_step(self):
        ...
        # …or the same hand-off without touching the laptop: a thumbs-up to the
        # webcam. The self-view is here so the child can see they are in frame;
        # its border goes green the moment the shape is recognised.
        cam_col = tk.Frame(btns, bg=BG_CARD)
        cam_col.pack(side="right")
        # Fixed box, not a sized Label: Label's width/height are text units
        # until an image lands in it, so the frame holds the pixels instead.
    ...
    # ── Learn Mode: thumbs-up hand-off ─────────────────────────────────────
    # Everything from here to _learn_done_my_part is the camera route through
    # the student's turn. "✓ I did it!" still works and still fires straight
    # away; a thumbs-up does the same job from across the room, with a 3-2-1
    # countdown in between so a mis-read hand can be taken back.
    ...
    def _one_finger_report(cls, lms):
        ...
        # Index out: must be straight enough AND clearly reaching away.
        # Both reach and extension required so a limp / half-open hand fails.
        ...
        # Index longer than every other fingertip (blocks open palm / V-sign).
        longer_n = 0
        ...
        longer_ok = longer_n >= 3  # must beat all three other tips

        # Middle, ring, pinky all shut — fold OR tuck with tight limits.
        # Extended open-hand fingers fail both (high PIP angle + long tuck).
        shut_n = 0
        for name, (mcp, pip, tip) in zip(
                ("middle", "ring", "pinky"),
        ...
        try:
            ...
            while not stop_evt.is_set():
                ...
                if not ok or frame is None:
                    time.sleep(0.04)
                    continue
                # Mirrored, so the self-view moves the way the child does.
                frame = cv2.flip(frame, 1)
                h, w = frame.shape[:2]
                scale = GESTURE_DETECT_W / float(max(1, w))
                small = (cv2.resize(frame, (GESTURE_DETECT_W,
                ...
                self._gesture_latest = (preview, thumbs)
                if stop_evt.wait(0.015):
                    break
        except Exception as e:
    ...
    def _gesture_start_watch(self):
        ...
        if not ok:
            ...
            self.log_to_console(f"Learn Mode: {reason}", "info")
            return
    ...
    def _gesture_poll(self):
        ...
        if latest is not None:
            ...
            if lbl is not None:
                try:
                    if lbl.winfo_exists():
                        ...
                        lbl.config(image=self._gesture_img,
            ...
            if thumbs:
                ...
                # Re-arm only once the hand is properly out of shape again, so
                # one held thumb can't fire twice on the way past a cancel.
    ...
    def _gesture_fired(self):
        ...
        self.log_to_console("Learn Mode: thumbs up detected.", "recv")
        self._gesture_show_hint("\U0001f44d Thumbs up detected!", ACCENT_GREEN)
        # The camera has done its job for this turn; the countdown owns the
        # screen from here, and holding the device open through it only risks
        # a second trigger landing mid-count.
        self._gesture_stop_watch()
        self._learn_start_countdown()

    # ── Learn Mode: 3-2-1 countdown ────────────────────────────────────────
    ...
    def _learn_start_countdown(self):
        ...
        if self._countdown_popup is not None:
            return
        if self._learn_student_idx is None:      # turn already moved on
            return
    ...
    def _learn_cancel_countdown(self):
        ...
        self._gesture_show_hint(
            ...
            ACCENT_AMBER)
        # Back to watching, but only if the student's turn is still on screen.
    ...
    # ── Gesture control tester (⚙ Settings → Gesture control test) ────────
    # Bench for camera gestures: thumbs-up (Learn Mode hand-off) and
    # one-finger-out (colour emptied). Live skeleton + every measurement.
    ...
    def _open_gesture_test_dialog(self):
        ...
        self._close_settings_popup()
        # Don't steal the camera mid colour-change; the emptied watcher needs it.
        if self._empty_gesture_thread is not None:
            self.show_hint_popup(
            ...
            try:
                ...
                while not state["stop"].is_set():
                    ...
                    try:
                        ...
                        if shown:
                            # Green if either gesture is recognised.
                            ...
                            for i, pt in enumerate(pts):
                                # Highlight index tip (8) for the one-finger pose.
        ...
        def tick():
            ...
            # One-finger panel (colour emptied)
        ...
        threading.Thread(target=worker, daemon=True).start()
        tick()
        # Map the window before raising it: Tk on macOS will walk into AppKit
        # with a half-built window otherwise.
        dlg.update_idletasks()
        try:
            dlg.lift()
            dlg.focus_force()
    ...
    def _learn_start_robot(self, idx):
        ...
        if self.port_var.get() and not self.is_sending:
            ...
            _SPEED_MAP = {"Aqua Low": 50, "Super Low (default)": 100, "Low": 150, "Medium": 200, "High": 250}
            f = _SPEED_MAP.get(self.feed_rate.get(), 100)
            lines = ["$X", "G21", "G90", f"F{f}"]
            # Head rests at Start; approach Origin before any drawing.
            lines += job_begin_from_start(TRAVEL_FEED)
            # Learn Mode's preview draws this part with the same Tk smooth
            # spline as the main canvas (see _learn_render_preview), so the
            # robot's line has to be fit to that curve too, not the sparse
            # generator points — see _gcode_source_paths.
    ...
    def _learn_start_symmetry_robot(self, pair):
        ...
        self._learn_streaming = False
        if self.port_var.get() and not self.is_sending:
            _SPEED_MAP = {"Aqua Low": 50, "Super Low (default)": 100,
                          "Low": 150, "Medium": 200, "High": 250}
            f = _SPEED_MAP.get(self.feed_rate.get(), 100)
            lines = ["$X", "G21", "G90", f"F{f}"]
            # Head rests at Start; approach Origin before any drawing.
            lines += job_begin_from_start(TRAVEL_FEED)
            # Same reasoning as _learn_start_robot: the symmetry preview is
            # also drawn with Tk's smooth spline, so smooth the clipped runs
            # before arc-fitting rather than fitting the raw control points.
            lines += self._paths_gcode_lines(
    ...
    def _learn_dot_gcode_lines(self, dots, f):
        ...
        for px, py in dots:
            ...
            # Travel with X and Y together — not drawing, just repositioning.
            lines += [
                f"G1 Z{NOZZLE_CLOSED_Z:.2f} F{Z_FEED}",
    ...
    def _learn_start_pulli(self):
        ...
        if self.port_var.get() and not self.is_sending:
            ...
            f = _SPEED_MAP.get(self.feed_rate.get(), 100)
            lines = ["$X", "G21", "G90", f"F{f}"]
            # Head rests at Start; approach Origin before laying dots.
            lines += job_begin_from_start(TRAVEL_FEED)
    ...
    # ── The live view ─────────────────────────────────────────────────────────
    # A still frame is no help when the job is aiming a camera at the floor, so
    # picking a camera opens its picture and leaves it running. The photo is
    # then grabbed straight out of that feed: what is framed is what is saved,
    # and the device is never opened twice at once (the second open fails).
    ...
    def _camera_view_opened(self, token, index, label, cap, frame):
        ...
        # The popup can be gone, or another camera picked, while the open was
        # still blocking. Either way this capture is no longer wanted.
        ...
        self._render_camera_devices()   # the row for this device now reads "live"
    ...
    def _camera_view_tick(self):
        ...
        try:
            ...
            if ok and frame is not None:
                ...
                fh, fw = frame.shape[:2]
                # Letterbox: the largest size that still fits the black box.
                ...
                # USB webcams drop the odd frame; only a long run of them means
                ...
                if self._camera_view_misses >= CAMERA_PREVIEW_MISSES:
                    ...
                    self._set_camera_status(
                        ...
                        "again.", ACCENT_PINK)
                    self._render_camera_devices()   # no row is "live" now
                    return
        except tk.TclError:
            self._camera_view_lbl = None
            self._stop_camera_view()
    ...
    def _open_learn_camera_step(self):
        ...
        # Every arrival here is a different finished rangoli, so whatever was
        # photographed for the last one must not count as this one's photo —
        # otherwise Continue would sail through on a stale image and the AI
        # would score the wrong mat.
        ...
        # Buttons first, pinned to the bottom, so the live view can take every
        # pixel left over without ever pushing them off the card.
        btns = tk.Frame(body, bg=BG_CARD)
        ...
        # The picture itself. The box is fixed; each frame is letterboxed into
        # it, so a 4:3 camera and a 16:9 one both come out un-stretched.
        # W-52 is the body width (see _learn_shell), so W-72 leaves a margin
        # either side at every UI scale.
        ...
        # The camera picker sits under the picture — one line per device once a
        # scan has run, so switching cameras is a single click.
        self._camera_list = tk.Frame(body, bg=BG_CARD)
        self._camera_list.pack(fill="both", expand=True)
        self._render_camera_devices()

        # A camera installed last run needs no second click: show its picture
        # straight away, so the step opens on the mat rather than on a button.
    ...
    def _render_camera_devices(self):
        ...
        for idx, label in self._camera_devices:
            ...
            if self._camera_view_ready():
                # On the camera step a click opens the picture, so say that
                # rather than "install" — installing is the side effect.
    ...
    def _start_camera_scan(self):
        ...
        if self._camera_scanning:
            return
        # The scan probes every index, including the one the live view is
        # holding open — and a device already open answers "busy", which the
        # scan cannot tell apart from "unplugged". So let go of it first.
    ...
    def _camera_scan_done(self, found):
        ...
        if found:
            ...
            self._render_camera_devices()
            # A scan that found something should end on a picture, not on a
            # list: show the installed camera, or the first one found, and let
            # a click on any other row switch to it.
            if self._camera_view_ready():
                idx, label = found[0]
                if self._camera_index is not None:
    ...
    def _close_learn_popup(self):
        ...
        # afterwards must not touch them. The live view holds an open device,
        # so it has to be released here or the camera stays locked.
        self._stop_camera_view()
        # The thumbs-up watcher holds the camera too, and its poll paints into
        # widgets this popup is about to take with it.
        self._close_countdown()
        self._gesture_stop_watch()
    ...
    def on_canvas_click(self, event):
        # Left-click always dismisses the shape context menu first.
    ...
    def _place_line_point(self, x, y, colour):
        ...
        for (mx, my), (mx2, my2) in zip(self._mirror_centers(x0, y0),
            ...
            self.shapes.append(dup)
        self.selected_shape_index = len(self.shapes) - 1
        # One line per pick, same rule as the single-click shapes — pick Line
        # again from Robot Test to draw another.
        self._finish_test_shape()
        self.redraw()
    ...
    def on_canvas_drag(self, event):
        if self._pending_hit is not None:
            ...
            if not self._drag_active:
                # Small dead zone so a plain click/tap still opens the edit
                # popup on release instead of being read as a tiny drag.
    ...
    def on_right_click(self, event):
        ...
        if closest_idx is not None and min_dist < 60:
            ...
            self.redraw()
            # Defer one tick so we are not posting UI from inside the
            # canvas bind dispatch (macOS Tk is sensitive to re-entrancy).
            self.root.after(
    ...
    def _scale_imported(self, s, new_size):
        ...
        s['paths'] = [[(cx + (x - cx) * f, cy + (y - cy) * f) for x, y in path]
                      for path in s['paths']]
        # A verbatim G-code job armed for this design (the Cmd+J rangoli, a
        # replayed job) describes the unscaled geometry, so sending it as-is
        # would plot the old size. Drop it: Send then regenerates from what is
    ...
    def _densify_smooth(self, pts, step_px=3.5):
        ...
        # Re-sample by true arc length: even pen speed, and even spacing for
        # the arc fitter downstream (see _resample_uniform).
    ...
    def redraw(self):
        ...
        # Every shape carries a colour now; kept as a parameter so the
        # per-shape draw helpers keep their signature.
        multi = True
    ...
    def get_shape_coords(self, s):
        ...
        if s['type'] == "Line":
            ...
            # The canvas draws a Circle with create_oval, so these points feed
            # G-code only. With CAL_X == CAL_Y the machine-space image of this
            # polygon is a true circle, so _fit_full_circle recognises it and
            # the whole shape leaves as one G2 — the sample count only has to
            # clear that check's ARC_MIN_RUN + 2 minimum, which anything
            # sensible does.
            #
            # It stays at 64 rather than dropping to 32 because the count is
            # also the fallback's accuracy floor: if the fast path ever declines
            # (a Circle scaled so small that rounding to 4 decimals moves a
            # point off the radius, say), _fit_arcs can only be as faithful as
            # the polyline it is handed, and a 32-gon's chords sit 0.51 mm
            # inside the true curve — past ARC_FIT_TOLERANCE, so the fitter
            # would emit them as straight G1s. 64 quarters that. Going denser
            # buys nothing: the arcs already track the curve, so extra points
            # only split them into more of them.
            CIRCLE_SEGMENTS = 64
    ...
    def _paths_gcode_lines(self, paths, f, z_closed=False, travel_f=None,
        ...
        for path in paths:
            ...
            # A path that is one true circle needs exactly one command. Try
            # that before the general fitter, which can only ever approximate
            # it with a chain of sub-360-degree arcs (see _fit_full_circle).
            ...
            # Work out both valve ramps before emitting anything, because the
            # header below has to know whether to still open the gate standing
            # still. A ramp needs a straight segment at that end of the path;
            # arcs decline, since splitting one means re-deriving its I/J
            # offsets against the radius check described above.
            #
            # A single-segment path is asked for both ramps at once so the one
            # length is judged against their sum — otherwise the open and the
            # close would each independently think they had room, and between
            # them eat the whole stroke.
            ...
            z_closed = False
            # Non-drawing reposition. After a refill, leave the dock the same
            # way we entered (Y up → X over → Y to the stroke start); otherwise
            # X+Y together at travel speed.
            ...
            for i, seg in enumerate(segs):
                if seg[0] == 'line':
                    ...
                    # Move the gate *inside* the stroke rather than in blocks
                    # of its own on either side of it. Folding Z into a G1 that
                    # is also travelling in XY makes GRBL pace the gate to
                    # start and finish with the head, so the line fades in and
                    # tapers out instead of being punched open and cut shut.
                    # Every drawing move restates F{f} so speed drops from
                    # TRAVEL_FEED the instant powder is meant to flow.
                    ...
                    if i == len(segs) - 1 and coast_pt is not None:
                        ...
                        # Next path can skip its leading close; if this did not
                        # fire, that close is still what shuts the gate.
                        z_closed = True
                    else:
    ...
    # Shape types the canvas actually renders with Tk's smooth=True spline
    # (see redraw() / _draw_complex_flower / _draw_preset_shape). Their raw
    # generator/import points are sparse control vertices, not the curve
    # itself — Tk's spline cuts the corner at every one of them. G-code built
    # straight from those raw points instead arc-fits the sharper, unsmoothed
    # polygon, so the robot's line quietly disagrees with what's on screen —
    # most visible on the few-point designs (Mandala Star, Petal Burst, ...).
    # Matching them here reuses the exact Bezier reconstruction already built
    # for the Simulate preview (_densify_smooth), so gcode traces the same
    # curve that's drawn/simulated instead of the bare control polygon.
    _SMOOTH_GCODE_TYPES = {"Flower", "Complex Flower", "Preset", "Imported", "Pen"}

    # Finer than the 3.5 px the Simulate preview uses. The preview only has to
    # look right on a 680 px canvas; the G-code sampling is the ceiling on how
    # accurate the fitted arcs can be, since _fit_arcs can only ever be as
    # faithful as the polyline it is handed. 3.5 px is 1.2 mm of chord on the
    # mat, far coarser than ARC_FIT_TOLERANCE, so the sampling — not the fit —
    # would set the error. Cost is nil: the arcs collapse these points away.
    GCODE_SAMPLE_PX = 1.5

    # Shapes whose points were traced, digitised or drawn by hand rather than
    # evaluated from a formula. Only these carry the sub-tolerance ripple that
    # makes the tangent-chained fit oscillate, so only these are filtered (see
    # _denoise_path). Running it over the generated shapes would add a little
    # bias to geometry that is already exact and fits in two arcs as it is.
    ...
    def _gcode_source_paths(self, s):
        ...
        if s.get('type') not in self._SMOOTH_GCODE_TYPES:
            return paths
        # Same length and order as `paths` — callers (e.g. generate_gcode's
        # multi-colour split) key colours by index into this list, so a path
        # can never be dropped or reordered here, only densified in place.
    ...
    def generate_gcode(self):
        _SPEED_MAP = {"Aqua Low": 50, "Super Low (default)": 100, "Low": 150, "Medium": 200, "High": 250}
        f = _SPEED_MAP.get(self.feed_rate.get(), 100)   # draw speed (Settings)
        tf = TRAVEL_FEED                                 # reposition / refill
        lines = ["$X", "G21", "G90", f"F{f}"]
        # Head always rests at Start; drive to Origin before any real work.
        ...
        if has_colour:
            ...
            for s in self.shapes:
                ...
                if path_colours:
                    ...
                    _add(s.get('colour'), gcode_paths)

            # The nozzle starts empty, so every colour costs a trip to its own
            # refill dock — LOADED_COLOUR is just drawn first. Stable
            # secondary order so Red always comes before Green when both
            # appear (matches left-to-right dock layout on the machine).
            ...
            # What the nozzle holds right now. Empty at job start, so the
            # first real colour always triggers a fill; a group only skips
            # the swap once that same colour is already loaded.
            in_nozzle = None
            for colour in ordered_colours:
                ...
                if needs_switch:
                    ...
                    if first_load:
                        # Nozzle starts empty — there is nothing to purge, so
                        # skip the trip to origin / valve-open / shake dance
                        # entirely and go straight to loading this colour.
                        lines += [
                            f";COLOUR_LOAD:{colour}:"
                            f"{dwell if dwell is not None else ''}"]
                    else:
                        # Travel to origin for purge (not drawing). The dwell
                        # (if this colour auto-refills) rides along on the
                        # marker so send_gcode can report the real fill time
                        # it's about to wait on, instead of a fixed guess.
                    ...
                    if dwell is not None:
                        # Drive to *this* colour's station: Y up → X over →
                        # Y down into the dispenser. The first stroke after
                        # the dwell leaves the same way (see
                        # sequential_from_refill).
                        rx, ry = dock
                        ...
                        leave_refill = True
                    in_nozzle = colour
                    # Leaving the dock the nozzle is already shut, so the first
                    # path can skip its own closing move.
                    z_closed = True
                else:
                ...
                for paths in groups[colour]:
                    ...
                    # Only the first path after a dock visit needs the safe
                    # leave sequence; later strokes of the same colour travel
                    # normally.
                    leave_refill = False
                    z_closed = False
        else:
            for s in self.shapes:
                lines += self._paths_gcode_lines(
                    self._gcode_source_paths(s), f,
                    travel_f=DRAW_APPROACH_FEED)

        # Done — rest at the start position (not the origin) with both axes.
        lines += [f"G1 Z0.00 F{Z_FEED}"] + start_position_g1(DRAW_APPROACH_FEED)
    ...
    def _show_send_preflight_popup(self):
        ...
        # Auto-confirm 10s after the popup opens if the operator doesn't
        # touch either button — the checklist is a reminder, not a gate the
        # operator is expected to sit and answer every job.
    ...
    def _purge_shake(self, ser, event):
        ...
        # Deliberately quiet: at tens of cycles a second, logging each one
        # would bury the colour-change messages in the console.
        ...
        # Always ends on the open move above, so the valve is left open — the
        # G1 Z{NOZZLE_CLOSED_Z} the G-code emits after ;COLOUR_SWITCH: is what
        # finally shuts it.
        return True
        ...
        try:
            ...
            for idx, clean in enumerate(lines):
                ...
                if clean.startswith(";COLOUR_LOAD:"):
                    ...
                    if auto:
                        ...
                        self.root.after(
                            ...
                            True)
                        # Nothing to purge, so just wait for the operator to
                        # confirm the colour is loaded — no gate shaking.
                        while not event.is_set():
                            self.pause_event.wait()
                            if self.cancel_requested:
                                grbl_ok = False
                ...
                if clean.startswith(";COLOUR_SWITCH:"):
                    ...
                    self.root.after(
                        0, self._arm_colour_emptied_button, event, colour)
                    # Shake instead of blocking outright: the gate chatters
                    # open and shut so the powder can't bridge and stall,
                    # and the loop checks the cancel flag every batch so
                    # Cancel can still break out of the colour prompt.
                    ...
                    continue
                if clean.startswith(";"):
                    # Comments mean nothing to GRBL; skip the write.
                    progress = (idx + 1) / total
                    self.progress_var.set(progress)
                    self.root.after(0, lambda p=progress: (
                        self.progress_bar.set(p),
...
if __name__ == "__main__":
    ...
    # Hard crashes (SIGSEGV/SIGABRT from Tk/AppKit) dump a Python stack when
    # possible. macOS still writes a .ips under ~/Library/Logs/DiagnosticReports.
    ...
    try:
        ...
        try:
            ...
            pass
        # Stop the sync thread and fold the write-ahead log back into
        # family_local.db, so the file left on disk is complete.
