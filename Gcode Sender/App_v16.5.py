...
GESTURE_POLL_MS      = 60    # UI-side drain of the detector thread's results
GESTURE_HOLD_FRAMES  = 5     # consecutive thumbs-up frames before it fires
GESTURE_CLEAR_FRAMES = 6     # consecutive clear frames before it can re-arm
GESTURE_PREVIEW_W    = 176   # self-view inside the Learn popup
GESTURE_PREVIEW_H    = 132
GESTURE_DETECT_W     = 320   # frames are shrunk to this before detection
# Shape thresholds, all scale-free (see _thumbs_up_from_landmarks).
# Every number below was read off real hands: thumbs-up, fist, V-sign and open
# palm photos put through the landmarker at eight rotations each (plus mirrored),
# then set between the two populations. The thumb's own straightness and reach
# are what actually separate a thumbs-up from a fist; the thumb-to-finger angle
# does not (a fist folds its thumb across the knuckles, which scores just as
# wide), so that one is set low, where it earns its keep throwing out the
# half-guessed landmark sets the model returns for a blurred background hand.
GESTURE_THUMB_STRAIGHT_DEG = 150.0   # min angle at the thumb's IP joint
GESTURE_THUMB_REACH        = 1.15    # thumb tip distance from wrist ÷ palm size
GESTURE_FINGER_CURL_DEG    = 130.0   # max angle at a folded finger's PIP joint
GESTURE_FINGER_TUCK        = 0.72    # folded tip distance from its MCP ÷ palm
GESTURE_THUMB_SPLAY_DEG    = 22.0    # min angle between thumb and finger axis
# Colour-emptied hand-off: hold up one finger (index out, other three shut) —
# same confirm as the 🎨 Emptied button / colour-switch popup. Same hold-frame
# idea as thumbs-up so a hand waving through the frame doesn't fire.
# Tuned from real pointing poses in the gesture tester (slight PIP bend is
# normal when pointing at the floor/camera; 145° was rejecting real hands).
GESTURE_INDEX_STRAIGHT_DEG = 115.0   # min angle at the index PIP (finger out)
GESTURE_INDEX_REACH        = 1.00    # index tip distance from wrist ÷ palm
GESTURE_INDEX_EXTEND       = 0.70    # tip away from MCP (fist tuck is ≤ ~0.72)
...
MANUAL_SECTIONS = [
    {
        ...
        "steps": [
            ("Plug in", "Connect the robot's USB cable to the laptop and "
                        "switch the robot on."),
            ("Pick the port", "Open ⚙ Settings in the top-left and choose "
            ...
            ("Fill the funnel", "Put the rangoli powder in the funnel and "
                                "check it flows when the valve opens."),
            ("Draw something", "Place a design on the canvas, then press "
                               "Send to Robot in the bottom strip."),
        ],
        "tips": [
            "Nothing is sent to the robot until you press Send to Robot. "
            "Placing and moving designs on the canvas is always safe.",
    ...
    },
    {
        ...
        "steps": [
            ("Open it", "Click ✎ Designs on the canvas, then "
                        "Pre-designed. The gallery also opens on its own when "
                        "the app starts."),
    ...
    },
    {
        ...
        "steps": [
            ("Open it", "✎ Designs → Import Designs."),
    ...
    },
    {
        ...
        "steps": [
            ...
            ("Click the canvas", "Click where the shape should go. A Line "
                                 ...
                                 "point."),
            ("Send it", "Press Send to Robot. Watch the first few "
                        "centimetres — that is where a flow problem shows."),
    ...
    },
    {
        ...
        "summary": "Move the robot head by hand — jog left/right/up/down, "
                   "or type absolute X/Y coordinates.",
        "steps": [
            ...
            ("Go to a point", "Type X and Y in machine units, then press Go. "
                             ...
                             "or the Refill dock."),
            ("Close when done", "Close the pad before Send to Robot — Manual "
                                "Drive holds the serial port while it is open."),
        ],
        "tips": [
            "The nozzle stays closed the whole time. Manual Drive is for "
            "positioning, not for laying powder.",
            "Coordinates are the same machine units as the canvas grid "
            "(bed ~3.8 × 3.8; the refill station sits past the right edge).",
            "Home (or the H key) drives back to the origin X0 Y0. "
            "Start is a separate park at X0 Y-0.3 (same Y-axis line as "
            "origin, slightly off the bed). Jobs always begin at Start, "
            "drive to Origin before any work, and return to Start when "
            "finished.",
        ],
    },
    {
        "id": "learn", "icon": "\U0001f393", "title": "Learn Mode",
        "summary": "The robot and the learner take turns: it draws a part, "
                   "you copy it by hand with the powder bottle.",
        "steps": [
            ("Start it", "Features → Learn Mode → Start."),
            ...
            ("Take your turn", "Copy the next part by hand with the powder "
                               "bottle, then press the button to say you are "
                               "done."),
    ...
    },
    {
        ...
        "tips": [
            ...
            "Saved pages show up in the gallery, in Learn Mode, and in Kolam "
            "of the Day.",
        ],
    },
    {
    ...
    },
    {
        ...
        "steps": [
            ...
            ("Lines are faint or broken", "Powder problem, not software: "
                                          "check the funnel is not bridged "
                                          "and try a slower speed."),
        ...
        "tips": [
            ...
            "Nothing is ever sent to the robot without pressing Send to "
            "Robot.",
        ],
    },
]
...
#
# The dwell isn't fixed: it's sized to how much colour that group of strokes
# actually needs. Measured on the machine: a 240 mm line takes one teaspoon,
# and the dispenser delivers about six teaspoons in two seconds (3 tsp/s).
# Combined that's ~720 mm of line covered per second of dwell. So the fill
# time is the group's total drawn length, in mm, divided by
# REFILL_MM_PER_TEASPOON, divided by the dispenser's rate — see
# refill_dwell_seconds().
REFILL_MM_PER_TEASPOON = 240.0 / 1.0      # mm of line one teaspoon covers
REFILL_TEASPOONS_PER_SEC = 6.0 / 2.0      # dispenser output rate
# Floor so a very short colour group still gets a real valve pulse instead of
# a near-zero G4 that the controller effectively skips.
REFILL_MIN_DWELL_S = 0.3


def refill_dwell_seconds(length_mm):
    """Seconds to hold the dispenser open to cover `length_mm` of drawing."""
    teaspoons_needed = length_mm / REFILL_MM_PER_TEASPOON
    return max(REFILL_MIN_DWELL_S,
               teaspoons_needed / REFILL_TEASPOONS_PER_SEC)
...
COLOUR_REFILL = {
    "Red":    (2.600,  0.000),   # first station
    "Green":  (3.000, -0.050),   # second station
    "Yellow": (3.450,  0.050),   # third station
}
...
class ShapeApp:
    def __init__(self, root):
        ...
        self._gesture_run      = 0      # consecutive thumbs-up frames seen
        ...
        self._empty_gesture_run    = 0     # consecutive hold frames
        self._empty_gesture_error  = None
        self._empty_gesture_lbl    = None  # optional status line on the popup
        self._colour_lengths_dlg = None  # ⚙ Settings → Colour Lengths popup
        self._colour_lengths_unit = "mm"  # remembered across opens
        # Colour Lengths sliders: 0.1-2.0x, applied to that colour's real
        # dwell. Per-colour so e.g. Green can run a different fill time from
        # Red/Yellow. Missing colours fall back to _REFILL_TIME_SCALE_DEFAULT.
        self._REFILL_TIME_SCALE_DEFAULT = 0.45
        self.refill_time_scale = {"Green": 0.50}
    ...
    def _build_canvas_overlays(self, host):
        ...
        # Progress & Impact lives inside the Features popup now, so the
        # toolbar keeps to Features / Designs.
    ...
    def _open_settings_popup(self):
        ...
        # 3a) Colour Lengths — real mm of drawing per colour on the canvas.
        slot = _row("Colour Lengths",
                    "Red / green / yellow drawing length in mm for the "
                    "design currently on the canvas.")
        self._color_button(slot, "\U0001f4cf Check", self._open_colour_lengths_dialog,
    ...
    def _open_features_popup(self):
        ...
        if self._features_popup is not None:
            ...
            return
        self.root.update_idletasks()
        # Four rows: Learn (which carries the Progress button too), Picture to
        # Rangoli, Kolam Notebook, Kolam of the Day. Capped to the screen so
        # the last row is never cut off on a short laptop display — the body
        # scrolls.
        W, H = S(400), min(S(620), self.root.winfo_screenheight() - S(40))
        sx = self.root.winfo_screenwidth()  // 2 - W // 2
        sy = self.root.winfo_screenheight() // 2 - H // 2
    ...
    def _show_colour_switch_popup(self, colour, first_load=False):
        ...
        # Gesture alternative to the click: hold one finger out to the camera.
        # Status is updated by _empty_gesture_poll.
        self._empty_gesture_lbl = tk.Label(
            body,
            text=("\U0001f446 Or hold one finger out to the camera "
                  f"(index) \u2014 0/{GESTURE_HOLD_FRAMES}"),
            bg=BG_CARD, fg=ACCENT_CYAN, font=("Segoe UI", FS(9), "bold"),
    ...
    def _empty_gesture_start_watch(self):
        ...
        self._empty_gesture_set_status(
            f"\U0001f446 Or hold one finger out to the camera "
            f"(index) \u2014 0/{GESTURE_HOLD_FRAMES}",
            ACCENT_CYAN)
        ...
        try:
            ...
            while not stop_evt.is_set():
                ...
                try:
                    ...
                    for hand in (res.hand_world_landmarks or []):
        ...
        if latest is not None:
            if latest:
                ...
                if held >= n:
                    ...
                    self._empty_gesture_set_status(
                        "\u2705 One finger — colour emptied!", ACCENT_GREEN)
                    self._empty_gesture_stop_watch()
                    self._on_colour_emptied_click()
                    return
                self._empty_gesture_set_status(
                    f"\U0001f446 One finger out \u2014 hold {held}/{n}",
                    ACCENT_GREEN)
            else:
                self._empty_gesture_run = 0
                self._empty_gesture_set_status(
    ...
    def _thumbs_up_report(cls, lms):
        ...
        _thumbs_up_from_landmarks is just the "ok" of this — the rest is what
        the ⚙ Settings tester puts on screen, so the numbers being judged are
        always the numbers that decided.
        """
        ...
        note("thumb straight", thumb_ip, GESTURE_THUMB_STRAIGHT_DEG,
             thumb_ip >= GESTURE_THUMB_STRAIGHT_DEG, "°")

        reach = cls._pt_dist(p[T_TIP], p[WRIST]) / palm
        note("thumb reach", reach, GESTURE_THUMB_REACH,
             reach >= GESTURE_THUMB_REACH)

        # index, middle, ring, pinky as (mcp, pip, tip)
        for name, (mcp, pip, tip) in zip(
                ("index", "middle", "ring", "pinky"),
                ((5, 6, 8), (9, 10, 12), (13, 14, 16), (17, 18, 20))):
            bend = cls._joint_angle(p[mcp], p[pip], p[tip])
            note(f"{name} folded", bend, GESTURE_FINGER_CURL_DEG,
                 bend <= GESTURE_FINGER_CURL_DEG, "°")
            tuck = cls._pt_dist(p[tip], p[mcp]) / palm
            note(f"{name} tucked", tuck, GESTURE_FINGER_TUCK,
                 tuck <= GESTURE_FINGER_TUCK)
        ...
        note("thumb splay", splay, GESTURE_THUMB_SPLAY_DEG,
             splay >= GESTURE_THUMB_SPLAY_DEG, "°")

        return {"ok": all(t[3] for t in tests), "tests": tests}
    ...
    def _one_finger_report(cls, lms):
        ...
        Index extended; middle, ring and pinky shut. Thumb is left free so a
        natural point still scores — the three folded fingers are what stop
        an open palm or a V-sign from counting. Same world-landmark, scale-free
        approach as the thumbs-up tests.
        """
        blank = {"ok": False, "tests": []}
        if lms is None or len(lms) < 21:
            return blank
        ...
        # Middle, ring, pinky shut — same fold/tuck numbers as thumbs-up.
        for name, (mcp, pip, tip) in zip(
                ("middle", "ring", "pinky"),
            ...
            detector = vision.HandLandmarker.create_from_options(
                vision.HandLandmarkerOptions(
                    base_options=mp_python.BaseOptions(
                        model_asset_path=HAND_MODEL_FILE),
                    num_hands=1,
                    # Default confidences let a blurred hand in the background
                    # through as a mess of landmarks that can score as anything.
    ...
    def _gesture_poll(self):
        ...
        A single good frame is not a hand-off — GESTURE_HOLD_FRAMES in a row
        are, which is roughly half a second of a deliberately held thumb and
        keeps a hand passing through the frame from starting the robot.
        """
        self._gesture_after = None
        ...
        if latest is not None:
                ...
                try:
                    if lbl.winfo_exists():
                        ...
                        lbl.config(image=self._gesture_img,
                                   highlightbackground=(ACCENT_GREEN if thumbs
                                                        else BG_CARD))
            ...
            if thumbs:
                ...
                self._gesture_run += 1
                if self._gesture_armed and self._gesture_run >= GESTURE_HOLD_FRAMES:
    ...
    def _open_gesture_test_dialog(self):
        ...
        state = {"stop": threading.Event(), "frame": None,
                 ...
                 "run": 0, "one_run": 0, "err": None, "after": None, "img": None}
        ...
        tk.Label(pad, text="Two gestures on the same camera feed:\n"
                           ...
                           "(hold still; green when every joint test passes).\n"
                           "\u2022 \U0001f446 One finger out — colour emptied "
                           "(index pointing, other fingers closed).",
                 bg=BG_CARD, fg=TEXT_DIM, font=("Segoe UI", FS(9)),
        ...
        for name in ("thumb straight", "thumb reach",
                     "index folded", "index tucked", "middle folded",
                     "middle tucked", "ring folded", "ring tucked",
                     "pinky folded", "pinky tucked", "thumb splay"):
        ...
        for name in ("index straight", "index reach", "index extended",
                     "middle folded", "middle tucked", "ring folded",
                     "ring tucked", "pinky folded", "pinky tucked"):
            r = tk.Frame(side, bg=BG_CARD)
            r.pack(fill="x", pady=S(1))
            tk.Label(r, text=name, bg=BG_CARD, fg=TEXT_DIM, width=S(15),
                     anchor="w", font=("Segoe UI", FS(9))).pack(side="left")
        ...
        def tick():
            ...
            if report is None:
                ...
                if report["ok"]:
                    state["run"] += 1
                    verdict.config(text="\U0001f44d THUMBS UP", fg=ACCENT_GREEN)
                    hold.config(
                        text=f"held {state['run']}/{GESTURE_HOLD_FRAMES} "
                             f"frames — " +
            ...
            if one is None:
                ...
                if one["ok"]:
                    state["one_run"] += 1
                    one_verdict.config(
                        text="\U0001f446 ONE FINGER OUT", fg=ACCENT_GREEN)
                    one_hold.config(
                        text=f"held {state['one_run']}/{GESTURE_HOLD_FRAMES} "
                             f"frames — " +
                             ("would confirm colour emptied now"
                              if state["one_run"] >= GESTURE_HOLD_FRAMES
                              else "keep holding"))
                else:
                    state["one_run"] = 0
    ...
    # mm per unit, for the Colour Lengths dialog's unit switcher.
    _LENGTH_UNITS = {"mm": 1.0, "cm": 10.0, "in": 25.4, "ft": 304.8, "m": 1000.0}
    ...
    def _colour_lengths_mm(self):
        """Total real-mm drawing length per colour for every shape currently
        on the canvas — used by the Settings 'Colour Lengths' button."""
        return {colour: self._colour_group_length_mm(paths)
                for colour, paths in self._colour_path_groups().items()}

    def _colour_lengths_and_load_times(self):
        """Per-colour (length_mm, hopper_load_seconds, draw_seconds) for the
        current canvas.

        hopper_load_seconds is refill_dwell_seconds(length_mm) — the dwell
        the automatic colour loader holds the valve open for at its dock,
        sized by REFILL_MM_PER_TEASPOON (a real ruler-on-mat calibration, so
        it needs the accurate real-mm length this method feeds it) — scaled
        by that colour's Colour Lengths slider (self.refill_time_scale).

        draw_seconds is separate: how long the robot spends actually moving
        the nozzle along that colour's path once loaded, i.e. raw machine-
        unit distance (the same space GRBL's own F feed rate is in) divided
        by the current Settings → Speed feed rate. This is real robot time
        that isn't spent at the refill dock at all, so it's reported next to
        — not combined with — the load time.
        """
        _SPEED_MAP = {"Aqua Low": 50, "Super Low (default)": 100,
                      "Low": 150, "Medium": 200, "High": 250}
        feed = _SPEED_MAP.get(self.feed_rate.get(), 100)  # machine units / min
        out = {}
        for colour, paths in self._colour_path_groups().items():
            units = self._colour_group_length_units(paths)
            mm = units * (MAT_SPAN_MM / MAX_X)
            draw_s = (units / feed) * 60.0 if feed else 0.0
            out[colour] = (mm, refill_dwell_seconds(mm) * self._refill_scale_for(colour), draw_s)
        return out

    # Colour Lengths sliders are calibrated at Super Low (default), feed=100.
    # Every other Settings → Speed tier is 50 feed-units apart and adds/removes
    # 20% loading time per step relative to that baseline: Aqua Low (50) is
    # 80%, Low (150) is 120%, Medium (200) is 140%, High (250) is 160%.
    _SPEED_MAP_FEED = {"Aqua Low": 50, "Super Low (default)": 100,
                        "Low": 150, "Medium": 200, "High": 250}

    def _speed_fill_multiplier(self):
        feed = self._SPEED_MAP_FEED.get(self.feed_rate.get(), 100)
        return 1.0 + (feed - 100) / 50.0 * 0.20

    def _refill_scale_for(self, colour):
        base = self.refill_time_scale.get(colour, self._REFILL_TIME_SCALE_DEFAULT)
        return base * self._speed_fill_multiplier()

    def _open_colour_lengths_dialog(self):
        """Settings → Colour Lengths: shows real-mm length per colour.

        Uses the app's own overrideredirect Toplevel style (like Manual
        Drive) rather than tkinter.messagebox — a native messagebox is
        modal (grab_set) but renders behind the Settings popup's own
        topmost overrideredirect window, so it was invisible while still
        blocking every click app-wide, forcing a force-quit."""
        existing = getattr(self, "_colour_lengths_dlg", None)
        if existing is not None:
            try:
                existing.lift()
                existing.focus_force()
        ...
        lengths_times = self._colour_lengths_and_load_times()
        order = {name: i for i, name in enumerate(COLOUR_REFILL)}
        rows = sorted(lengths_times.items(),
                       key=lambda kv: (order.get(kv[0], 99), kv[0]))

        W = S(340)
        H = S(140) + S(100) * max(1, len(rows))
        sx = self.root.winfo_screenwidth() // 2 - W // 2
        sy = max(S(10), self.root.winfo_screenheight() // 2 - H // 2)
        ...
        dlg.configure(bg=BG_DARK)
        dlg.transient(self.root)
        self._colour_lengths_dlg = dlg

        glass = tk.Canvas(dlg, width=W, height=H, bg=BG_DARK, highlightthickness=0)
        glass.pack(fill="both", expand=True)
        self._draw_rounded_rect(glass, 4, 4, W - 4, H - 4, radius=S(20),
                                fill=BG_CARD, outline=ACCENT_AMBER, width=2)
        glass.create_text(24, 26, text=self.tr("Colour Lengths"), anchor="w",
                          fill=TEXT_PRIMARY, font=("Segoe UI", FS(14), "bold"))

        body = tk.Frame(dlg, bg=BG_CARD)
        body.place(x=20, y=54, width=W - 40, height=H - 90)

        unit_row = tk.Frame(body, bg=BG_CARD)
        unit_row.pack(fill="x", pady=(S(0), S(10)))
        tk.Label(unit_row, text=self.tr("Unit"), bg=BG_CARD, fg=TEXT_PRIMARY,
                 font=("Segoe UI", FS(11), "bold")).pack(side="left")
        unit_var = tk.StringVar(value=self._colour_lengths_unit)
        unit_combo = ctk.CTkComboBox(
            unit_row, variable=unit_var, values=list(self._LENGTH_UNITS),
            state="readonly", width=S(100), fg_color=BG_INPUT,
            border_color=GLASS_EDGE, button_color=GLASS_EDGE,
            button_hover_color=ACCENT_AMBER, text_color=TEXT_PRIMARY,
            dropdown_fg_color=BG_CARD, dropdown_text_color=TEXT_PRIMARY,
            font=("Segoe UI", FS(11)))
        ...
        # Base (unscaled) mm + draw_s per colour — the dwell is recomputed
        # fresh from mm every render, driven by that colour's own slider,
        # instead of baking in whatever scale applied at dialog-open time.
        base_rows = [(colour, mm, draw_s) for colour, (mm, _dwell_s, draw_s) in rows]

        rows_frame = tk.Frame(body, bg=BG_CARD)
        rows_frame.pack(fill="both", expand=True)

        def render(*_):
            unit = unit_var.get()
            self._colour_lengths_unit = unit
            mm_per_unit = self._LENGTH_UNITS[unit]
            for w in rows_frame.winfo_children():
                w.destroy()
            if not base_rows:
                tk.Label(rows_frame, text=self.tr("Nothing on the canvas yet."),
                         bg=BG_CARD, fg=TEXT_DIM,
                         font=("Segoe UI", FS(11))).pack(anchor="w", pady=S(4))
            decimals = 0 if mm_per_unit <= 10.0 else 2
            for colour, mm, draw_s in base_rows:
                value = mm / mm_per_unit
                row = tk.Frame(rows_frame, bg=BG_CARD)
                row.pack(fill="x", pady=S(6))
                tk.Label(row, text=f"{colour}: {value:.{decimals}f} {unit}",
                         bg=BG_CARD, fg=TEXT_PRIMARY,
                         font=("Segoe UI", FS(12), "bold")).pack(anchor="w")

                dwell_lbl = tk.Label(row, bg=BG_CARD, fg=TEXT_DIM,
                                     font=("Segoe UI", FS(9)))
                dwell_lbl.pack(anchor="w")
                tk.Label(row, text=self.tr(
                             f"Draw time: ~{draw_s:.1f} s (nozzle, at current Speed)"),
                         bg=BG_CARD, fg=TEXT_DIM,
                         font=("Segoe UI", FS(9))).pack(anchor="w")

                slider_row = tk.Frame(row, bg=BG_CARD)
                slider_row.pack(fill="x", pady=(S(4), S(0)))
                tk.Label(slider_row, text=self.tr("Fill time @ Super Low"), bg=BG_CARD,
                         fg=TEXT_PRIMARY, font=("Segoe UI", FS(9), "bold")
                         ).pack(side="left")
                pct_lbl = tk.Label(slider_row, text="", bg=BG_CARD,
                                   fg=ACCENT_AMBER, font=("Segoe UI", FS(9), "bold"))
                pct_lbl.pack(side="right")

                # Slider holds the base per-colour % (set at Super Low);
                # the speed multiplier is applied on top when computing the
                # displayed/real dwell, not baked into the slider value —
                # otherwise it would compound every time Speed changes.
                base_pct = self.refill_time_scale.get(
                    colour, self._REFILL_TIME_SCALE_DEFAULT) * 100.0
                scale_var = tk.DoubleVar(value=base_pct)

                def make_update_dwell(colour, mm, dwell_lbl, pct_lbl, scale_var):
                    def update_dwell(_v=None):
                        pct = round(scale_var.get())
                        base_scale = pct / 100.0
                        self.refill_time_scale[colour] = base_scale
                        pct_lbl.configure(text=f"{pct}%")
                        dwell_s = refill_dwell_seconds(mm) * self._refill_scale_for(colour)
                        dwell_lbl.configure(text=self.tr(
                            f"Hopper load: ~{dwell_s:.1f} s (auto colour loader)"))
                    return update_dwell

                update_dwell = make_update_dwell(colour, mm, dwell_lbl, pct_lbl, scale_var)
                scale_slider = ctk.CTkSlider(
                    row, from_=10, to=200, number_of_steps=190,
                    variable=scale_var, fg_color=BG_INPUT,
                    progress_color=ACCENT_AMBER, button_color=ACCENT_AMBER,
                    button_hover_color=ACCENT_AMBER, height=S(14))
                scale_slider.pack(fill="x", pady=(S(2), S(0)))
                scale_slider.configure(command=update_dwell)
                update_dwell()

        unit_combo.configure(command=render)
        render()

        def close():
            self._colour_lengths_dlg = None
            try: dlg.destroy()
            except Exception: pass
    ...
    def _colour_group_length_units(self, path_groups):
        """Total drawn length, in raw machine units (GRBL's own coordinate
        space — the same units its F feed rate is denominated in), of a
        colour's queued paths. `path_groups` is a list of path-lists as
        stored in generate_gcode's `groups[colour]`."""
        total = 0.0
        for paths in path_groups:
            for path in paths:
                mpts = [self.to_machine(px, py) for px, py in path]
                for (x0, y0), (x1, y1) in zip(mpts, mpts[1:]):
                    total += math.hypot(x1 - x0, y1 - y0)
        return total

    def _colour_group_length_mm(self, path_groups):
        """Total drawn length, in real mm, of a colour's queued paths.

        Feeds refill_dwell_seconds() so the auto-refill valve opens only as
        long as this colour actually needs.
        """
        mm_per_unit = MAT_SPAN_MM / MAX_X
        return self._colour_group_length_units(path_groups) * mm_per_unit
            ...
            for colour in ordered_colours:
                ...
                if needs_switch:
                    ...
                    if self.auto_colour_var.get() and dock is not None:
                        length_mm = self._colour_group_length_mm(groups[colour])
                        dwell = refill_dwell_seconds(length_mm) * self._refill_scale_for(colour)
                    if first_load:
    ...
    def _purge_shake(self, ser, event):
        ...
        place of simply waiting for the Emptied button. Held wide open the
